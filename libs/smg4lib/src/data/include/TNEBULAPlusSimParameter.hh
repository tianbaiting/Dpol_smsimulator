#ifndef TNEBULAPLUSSIMPARAMETER_HH
#define TNEBULAPLUSSIMPARAMETER_HH
...
class TNEBULAPlusSimParameter : public TSimParameter
{
public:
  TNEBULAPlusSimParameter(TString name="NEBULAPlusParameter");
  virtual ~TNEBULAPlusSimParameter();

  TDetectorSimParameter *FindDetectorSimParameter(Int_t id);

  virtual void Print(Option_t* option="") const;// interited from TNamed
  friend std::ostream& operator<<(std::ostream& out,  const TNEBULAPlusSimParameter& prm);
...
public:
  ...
  //       ID  parameter
  std::map<int,TDetectorSimParameter> fNEBULAPlusDetectorParameterMap;

  ClassDef(TNEBULAPlusSimParameter, 1)
};
