...
ClassImp(TNEBULAPlusSimParameter)
//____________________________________________________________________
TNEBULAPlusSimParameter::TNEBULAPlusSimParameter(TString name)
: TSimParameter(name),
  fIsLoaded(false),
...
{
  std::cout<<"TNEBULAPlusSimParameter"<<std::endl;
}
//____________________________________________________________________
TNEBULAPlusSimParameter::~TNEBULAPlusSimParameter()
{;}
//____________________________________________________________________
TDetectorSimParameter*  TNEBULAPlusSimParameter::FindDetectorSimParameter(Int_t id)
{
  std::map<int, TDetectorSimParameter>::iterator it
    = fNEBULAPlusDetectorParameterMap.find(id);
  if (it==fNEBULAPlusDetectorParameterMap.end()) return 0;
  return &(it->second);
}
//____________________________________________________________________
std::ostream& operator<<(std::ostream& out,  const TNEBULAPlusSimParameter& prm)
{
  out << prm.GetName() <<": "
...
//____________________________________________________________________
void TNEBULAPlusSimParameter::Print(Option_t*) const
{
  std::cout << GetName() <<": "
	    << "fIsLoaded="       <<fIsLoaded <<": "
	    << "fNeutNum="        <<fNeutNum <<" "
