...
void analysis_crosstalk_example(TString filename="root/examples/example_tree0000.root",
...
//  TArtCrosstalkAnalysis *ctana = new TArtCrosstalkAnalysis_DoNothing;
  ...
  TH1* hidq_cta = new TH2D("hidq_cta","ID Q cta",605,0.5,605.5, 200,0,200);
  ...
  TH1* hb0112q2_s     = new TH2D("hb0112q2_s","beta01/beta12 Q2 DIFF",200,-4,4, 200,0,150);

  TH1* hb01q1_d     = new TH2D("hb01q1_d","beta01 Q1 DIFF",200,-1.,1., 200,0,150);
  TH1* hb01q2_d     = new TH2D("hb01q2_d","beta01 Q2 DIFF",200,-1.,1., 200,0,150);
...
//    gNeuLANDSimDataArray->clear();
    ...
    for (Int_t ipla=0;ipla<multi_cta;++ipla){
      TArtNEBULAPla *pla = (TArtNEBULAPla*)NEBULAPlaArray->At(ipla);
      hidq_cta->Fill(pla->GetID(), pla->GetQAveCal());
    }
    ...
    if (ctana->GetDLayer()==0){// SAME Wall 
      ...
      hb01q1_d->Fill(ctana->GetBeta01(), ctana->GetQ1());
      hb01q2_d->Fill(ctana->GetBeta01(), ctana->GetQ2());
