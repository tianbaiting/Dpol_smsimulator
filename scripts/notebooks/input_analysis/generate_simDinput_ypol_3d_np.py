...
# do not rotate. 
#
# ypol is rotate in qmd data.
#
# in this do not rotate it back,                         
# \# Rotation operation 
#
#     angle_rad = 0
...
# Store source metadata for outputs
source_metadata = {
    "notebook": "/home/tian/workspace/dpol/smsimulator5.5/d_work/macros/inputana/generate_simDinput_ypol_3d_np.ipynb",
            ...
            with open(filepath, 'r') as inputfile:
                ...
                for line in inputfile:
                    ...
                    if condition1_part1 and condition1_part2:
                        ...
                        pxp, pyp, pzp = pp_orig[0], pp_orig[1], pp_orig[2]
                        pxn, pyn, pzn = pn_orig[0], pn_orig[1], pn_orig[2]

                        # Second condition check alway true
                        condition2_part1 = (pxp + pxn) < 200
                        # condition2_part1 = True
                        # condition2_part2 = (np.pi - abs(phi_for_rotation)) < 0.2
                        condition2_part2 = True
...
def plot_interactive_3d_momentum():
    ...
    # Attach metadata
    fig.update_layout(meta={'source_metadata': source_metadata})

    # Save as HTML with embedded metadata comment
    html_path = "output/ypol_origindata_slected_interactive_3d_momentum_by_target.html"
    html_str = pio.to_html(fig, full_html=True, include_plotlyjs='cdn')
    html_comment = f"<!-- source_metadata: {json.dumps(source_metadata)} -->\n"
    with open(html_path, "w", encoding="utf-8") as f:
        f.write(html_comment + html_str)
...
def plot_total_interactive_3d_momentum():
    ...
    # Save as HTML with embedded metadata comment
    html_path = "output/ypol_origindata_seleced_rotateback_3d_momentum.html"
    html_str = pio.to_html(fig, full_html=True, include_plotlyjs='cdn')
    html_comment = f"<!-- source_metadata: {json.dumps(source_metadata)} -->\n"
    with open(html_path, "w", encoding="utf-8") as f:
        f.write(html_comment + html_str)
    print(f"Total interactive 3D plot saved to: {html_path}")
...
def create_animated_3d_momentum():
    ...
    # Save as HTML with embedded metadata comment
    html_path = "output/ypol_origindata_animated_3d_momentum_gamma.html"
    html_str = pio.to_html(fig, full_html=True, include_plotlyjs='cdn')
    html_comment = f"<!-- source_metadata: {json.dumps(source_metadata)} -->\n"
    with open(html_path, "w", encoding="utf-8") as f:
        f.write(html_comment + html_str)
    print(f"Animated 3D plot saved to: {html_path}")
...
def plot_3d_momentum_by_target_gamma():
    ...
    # Paths
    pdf_path = 'output/ypol_origindata_selected_3d_momentum_by_target_gamma.pdf'
    png_path = 'output/ypol_origindata_selected_3d_momentum_by_target_gamma.png'

    # Save PDF with embedded metadata
    pdf_meta = {
        'Title': '3D Momentum Distribution for Different Targets and Gammas - Y Polarization',
        'Subject': 'Momentum distribution plots generated by notebook',
        'Keywords': f"source={source_metadata.get('notebook','')}; SMSIMDIR={source_metadata.get('SMSIMDIR','')}",
        'Author': os.getenv('USER', '')
    }
    with PdfPages(pdf_path, metadata=pdf_meta) as pdf:
        pdf.savefig(fig, bbox_inches='tight', dpi=300)

    # Save PNG with embedded metadata (PNG tEXt chunks)
    png_meta = {
        'Source': source_metadata.get('notebook', ''),
        'Generator': 'Matplotlib',
        'SMSIMDIR': str(source_metadata.get('SMSIMDIR', '')),
        'GeneratedAt': source_metadata.get('generated_at', ''),
    }
    fig.savefig(png_path, bbox_inches='tight', dpi=300, metadata=png_meta)
...
output_root_dir_native = os.path.join(smdir, "d_work/rootfiles/ypol_sle_rotate_back/")
