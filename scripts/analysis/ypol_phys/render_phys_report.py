...
""")
        ...
        for (t, g) in sorted(agg.keys()):
            cells = agg[(t, g)]
            line = f"{t} & {g}"
            for cn in ("loose", "mid", "tight"):
                for kind in ("truth_R", "reco_mixed_R", "reco_full_R"):
                    vals = [c[f"{cn}_{kind}"] for c in cells if not math.isnan(c[f"{cn}_{kind}"])]
                    line += " & " + (fmt(_s.mean(vals)) if vals else "—")
            line += r" \\" + "\n"
            tex.append(line)
        tex.append(r"""\bottomrule
\end{tabular}
...
\caption{{$\Delta p_x^\text{{rot}}$ 分布 ({tg}, {cn} cut)。黑 step = A truth, 橙填充 = B reco mixed, 蓝虚线 = C full reco.}}
\end{{figure}}
""")
...
\caption{{$\Delta p_x^\text{{rot}}$ 在四 cut 下的叠加 ({tg})。4 行=4 gammas, 列=truth (A) / reco mixed (B). 蓝=无 cut, 绿=loose, 橙=mid, 红=tight.}}
\end{{figure}}
""")
...
\begin{tabular}{lr}
\toprule
mean R\_truth\_tight (16 cells)            & {tval} \\
