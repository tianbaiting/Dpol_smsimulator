...
def main():
    ...
    # Aggregate over helicity: keys are (target, gamma) only.
    # Per (target, gamma, cut, kind) → list of Δpx
    by_key: dict = defaultdict(list)
    cell_cut_counts: dict = defaultdict(lambda: {"loose": 0, "mid": 0, "tight": 0, "raw": 0, "any_reco": 0, "full_reco": 0})

    for cp in cells:
        tag = cp.stem
        target, gamma, hel = cell_decode(tag)  # hel kept only for diagnostic; not used as key
        with open(cp) as f:
            for r in csv.DictReader(f):
                ...
                cell_cut_counts[(target, gamma)]["raw"] += 1
                ...
                for cut_name, ok in (("loose", loose), ("mid", mid), ("tight", tight)):
                    if not ok:
                        continue
                    cell_cut_counts[(target, gamma)][cut_name] += 1
                    by_key[(target, gamma, cut_name, "truth")].append(truth_dpx)
                    if not math.isnan(reco_mixed_dpx):
                        by_key[(target, gamma, cut_name, "reco_mixed")].append(reco_mixed_dpx)
                    if not math.isnan(reco_full_dpx):
                        by_key[(target, gamma, cut_name, "reco_full")].append(reco_full_dpx)
    ...
    for target in targets:
        for gamma in gammas:
            counts = cell_cut_counts[(target, gamma)]
            row = {
                "target": target,
                "gamma": gamma,
                "n_raw": counts["raw"],
            ...
            for cut_name in ("loose", "mid", "tight"):
                for kind in ("truth", "reco_mixed", "reco_full"):
                    diffs = by_key.get((target, gamma, cut_name, kind), [])
...
def plot_dpx_hist_per_cut(csv_dir, output_dir):
    ...
    One figure per (target, cut). Helicities aggregated together — panels =
    4 gammas × 1 column."""
    try:
        import matplotlib
        matplotlib.use("Agg")
    ...
    cells = sorted(p for p in csv_dir.glob("*.csv"))
    # (target, gamma) -> list of event-records (helicities pooled)
    ...
    for cut_name in cut_names:
        for target in targets:
            fig, axes = plt.subplots(len(gammas), 1,
                                     figsize=(7, 3.5 * len(gammas)),
                                     squeeze=False)
            for gi, gamma in enumerate(gammas):
                ax = axes[gi, 0]
                rows_cell = by_tg.get((target, gamma), [])
                tvals, rvals, fvals = [], [], []
                for r in rows_cell:
                ...
                if tvals:
                    ax.hist(tvals, bins=bins, range=rng, histtype="step",
                            ...
                            label=f"A truth (N={len(tvals)})")
                if rvals:
                    ax.hist(rvals, bins=bins, range=rng, histtype="stepfilled",
                            alpha=0.35, color="C1",
                            label=f"B reco mixed (N={len(rvals)})")
                if fvals:
                    ax.hist(fvals, bins=bins, range=rng, histtype="step",
                            color="C0", linewidth=1.4, linestyle="--",
                            label=f"C full reco (N={len(fvals)})")
                ax.set_xlabel(r"$\Delta p_x^\mathrm{rot}$ = rot_pxp − rot_pxn (MeV/c)")
                ax.set_ylabel("counts")
                ax.legend(fontsize=8)
                ax.set_title(f"{target} {gamma} [{cut_name}]  (ynp+ypn pooled)")
...
def plot_r_vs_gamma_per_cut(rows, output_dir):
    """Plot R vs gamma for each cut, comparing truth / reco mixed / reco full.
    Helicities are already pooled in `rows` (no helicity column)."""
    try:
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
    ...
    for cut_name in ("loose", "mid", "tight"):
        ...
        for ti, target in enumerate(targets):
            ...
            ax.plot(xs, truth_R, "o-", color="black", label="A truth", linewidth=1.8, markersize=8)
            ax.plot(xs, reco_R, "s--", color="C1", label="B reco mixed", linewidth=1.5, markersize=7)
            ax.plot(xs, full_R, "^:", color="C0", label="C full reco", linewidth=1.5, markersize=7)
            ax.set_xlabel("gamma")
            ax.set_ylabel(rf"$R_x^\mathrm{{rot}} = N(\Delta p_x>0)/N(\Delta p_x<0)$  [{cut_name} cut]")
            ax.set_title(f"{target}  (ynp+ypn pooled)")
