...
ensure_exists() {
    ...
    local label="$2"
    if [[ ! -e "${path}" ]]; then
        echo "[run_sn124_nn_reco_pipeline] missing ${label}: ${path}" >&2
        exit 1
    fi
}
...
echo "[run_sn124_nn_reco_pipeline] exporting NN model for C++ inference"
...
if [[ "${#INPUT_FILES[@]}" -eq 0 ]]; then
    echo "[run_sn124_nn_reco_pipeline] no g4input files selected" >&2
    exit 1
fi
...
echo "[run_sn124_nn_reco_pipeline] selected ${#INPUT_FILES[@]} g4input files"
...
echo "[run_sn124_nn_reco_pipeline] done"
echo "[run_sn124_nn_reco_pipeline] g4output: ${G4OUTPUT_BASE}"
echo "[run_sn124_nn_reco_pipeline] reconstruction: ${RECO_OUTPUT_DIR}"
