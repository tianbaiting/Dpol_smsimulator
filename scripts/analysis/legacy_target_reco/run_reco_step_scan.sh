#!/usr/bin/env bash
# [EN] Usage: bash scripts/analysis/legacy_target_reco/run_reco_step_scan.sh / [CN] 用法: bash scripts/analysis/legacy_target_reco/run_reco_step_scan.sh
set -uo pipefail
...
root -l -q "${SCRIPT_DIR}/compare_reco_step_sizes.C+(\"${SIM_ROOT}\", \"${SIM_MACRO}\", \"${FIELD_TABLE}\", 30.0, \"${STEP_SIZES}\", \"${COMPARE_ROOT}\", \"${COMPARE_CSV}\", 627.0, 5.0, 1000, 80, 0.05, 0.5, 5.0, -1)" \
