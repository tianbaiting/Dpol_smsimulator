...
#include "G4HadronPhysicsINCLXX.hh"//Geant4.9
...
QGSP_MENATE_R::QGSP_MENATE_R(G4int ver, const G4String&) : G4VModularPhysicsList()
{
  //G4DataQuestionaire it(photon, neutron, neutronxs); // G4.10.6
  G4cout << "<<< Geant4 Physics List simulation engine: QGSP_MENATE_R"<< G4endl;
  G4cout << G4endl;
...
//RegisterPhysics( new HadronPhysicsQGSP_INCLXX(ver));
  ...
  // MENATE_R
  RegisterPhysics( new MenatePhysics(ver, "ORIGINAL") );

  // SAMURAICutsPhysics
  RegisterPhysics( new SAMURAICutsPhysics(ver) );
}

QGSP_MENATE_R::~QGSP_MENATE_R()
{;}

void QGSP_MENATE_R::SetCuts()
{
  SetCutsWithDefault();   
}
