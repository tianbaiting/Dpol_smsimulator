...
\begin{frame}{Isotope Reference: $^{112}$Sn versus $^{124}$Sn}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \begin{block}{$^{124}$Sn}
        Primary neutron-rich sensitivity target.  Same-chain y-pol tight px60
        full-reco closure is available and shown in the main result.
      \end{block}
      \begin{block}{$^{112}$Sn}
        Isotope reference target.  It tests whether the response follows target
        neutron excess rather than only generic kinematics or detector effects.
      \end{block}
    \end{column}
    \begin{column}{0.52\textwidth}
      \begin{block}{Current audit}
        The production description includes Sn112 + Sn124, but the locally
        exported tight-px60 folded tables are Sn124-only.
      \end{block}
      \begin{block}{Consequence}
        A direct quantitative isotope plot is not shown in the main result yet.
        Full folded Sn112 closure with the identical chain remains to be
        completed.
      \end{block}
    \end{column}
  \end{columns}
  \centering
  \footnotesize
  A common detector effect should influence both isotopes; an isovector response
  should change with target neutron excess.
\end{frame}
    ...
    \begin{column}{0.50\textwidth}
      \begin{block}{Still required}
        \begin{itemize}
          ...
          \item full Sn112/Sn124 common-chain closure,
...
\begin{frame}{Backup E: $^{112}$Sn Physics-Motivation Reference}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.50\textwidth}
      \centering
    ...
    \begin{column}{0.50\textwidth}
      \small
      Existing Sn112 information is useful as a physics-motivation reference,
      but not as a direct quantitative comparison with the current Sn124
      NEBULA Plus closure.
      \begin{block}{Reason}
        The locally exported tight-px60 folded tables are Sn124-only; older
        Sn112 ROOT files come from a different reconstruction production.
      \end{block}
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{Backup E: Sn112/Sn124 Consistency Audit}
  \scriptsize
  \begin{table}
    \centering
    \begin{tabular}{lll}
      ...
      y-pol $\gpar$ grid & 0.5,0.6,0.7,0.8 & intended same \\
  ...
  \begin{block}{Decision}
    Do not show a direct quantitative isotope plot until Sn112 is exported and
    checked with the identical chain.
  \end{block}
\end{frame}
