...
\title[190 MeV/u d on Sn124]{190 MeV/u Polarized Deuteron on $^{124}$Sn\\Local Full-Chain Rate Estimate}
\subtitle{Only locally reproducible ImQMD + Geant4 results are retained}
\author{Tian}
\date{2026-04-12}
...
\small
\begin{itemize}
  \item This English report keeps only \textbf{locally reproducible} results.
  \item All headline numbers come from the current local chain:
  \begin{itemize}
  ...
  \end{itemize}
  \item Results quoted from \texttt{polarization.tex} are intentionally removed from the main result tables.
  \item The present quantitative scope is:
  \begin{itemize}
    \item target: $^{124}$Sn,
...
\begin{frame}{Inputs Used in the Local Calculation}
\small
\begin{itemize}
...
\vspace{0.1cm}
\small
The local full-chain quantities taken from \texttt{validation\_summary.json} are:
\begin{itemize}
...
\begin{frame}{How the Bunch Interval Is Calculated}
\small
\begin{align*}
...
\small
So the bunch interval is obtained from beam kinematics and SRC machine parameters, not from an assumed timing constant.
\end{frame}

\begin{frame}{How the Production and Coincidence Cross Sections Are Calculated}
\tiny
\textbf{Step 1: ImQMD breakup probability in each shell}
\[
...
\begin{itemize}
  \item The main coincidence rate in this report is \textbf{not} obtained from a geometry-only solid-angle factor.
  \item The detector geometry is defined by the current Geant4 macro, but the quoted coincidence rate comes from \textbf{Geant4 hit counting} in that geometry.
...
\begin{frame}{Current Local Full-Chain Statistics}
\small
\centering
...
\vspace{0.2cm}
\small
The Geant4 coincidence efficiency is stable at the level of $\sim 0.03$ percentage points between the fresh rerun and the archived output.
\end{frame}

\begin{frame}{Cross Sections Obtained from the Local Chain}
\small
\centering
...
\vspace{0.25cm}
\begin{itemize}
  \item The recommended result for the present \texttt{b\_discrete} data structure is the shell-sum estimate, not the naive area formula.
  \item For rate conversion in this report:
  \[
...
\begin{frame}{Where the Coincidence Rate Comes From}
\small
\begin{itemize}
  \item The coincidence rate quoted in this report comes from the \textbf{full chain}:
  \[
  ...
  \end{itemize}
  \item What is \textbf{not} used as a headline result here:
  \begin{itemize}
    \item \texttt{polarization.tex} rate coefficients,
    \item geometry-only acceptance proxies,
    \item target placeholders without a same-chain extraction.
  \end{itemize}
\end{itemize}
\end{frame}

\begin{frame}{Headline Numbers from the Current Local Chain}
\small
\begin{itemize}
  ...
  \]
  \item These numbers are local full-chain results for the current $^{124}$Sn zpol $\gamma=0.60$ sample only.
\end{itemize}
\end{frame}
