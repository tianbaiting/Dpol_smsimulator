...
① QMD 模拟: d+Sn 190 MeV/u, $\gamma=\{0.5,0.6,0.7,0.8\}$, 靶 Sn112+Sn124, ypol (phi\_random) + zpol (b\_discrete)。 \\
② breakup 提取: \texttt{breakup\_nn\_targetframe\_20260607\_180428} (2026-06-07 18:04)。 \\
③ NEBULA-Plus + PDC NN joint 重建: \texttt{nebulaplus\_nn\_joint\_20260609\_012854} (2026-06-09 01:28)。 \\
所有图表脚本 (\texttt{make\_cut\_diagnostics\_figures.py}, \texttt{plot\_nebulaplus\_reco.py}) 用 uproot 直接读 ③ 的 176 个 \texttt{*\_reco.root} 文件 (\texttt{recoTree})，过滤 \texttt{truth\_has\_proton \& truth\_has\_neutron \& nn\_status==0}。Sn124 事件数: ypol $1.68\times10^6$ (16 文件)，zpol $3.75\times10^4$ (160 文件)。
...
\toprule
pol & $\gamma$ & $N_{+}$ & $N_{-}$ & $R_{\mathrm{folded}}$ & $\sigma(R)_{\mathrm{sim}}$ & $\sigma(R)_{16\mathrm{h}}$ \\
\midrule
...
$N_{+}$/$N_{-}$ 是 $\Delta p_x^{\mathrm{reco}}$ 正/负的事例数，$\sigma(R)_{\mathrm{sim}}$ 是当前 MC 样本的统计误差，$\sigma(R)_{16\mathrm{h}}$ 是外推到 16 h 束流时间后的统计误差 (公式见 \S 5)。
...
这三张是 \textbf{truth-level} 相空间密度 (事件计数，非效率)。tight px60 把中子 $|p_{x,n}|<60$ 的窄条切出来，$\Delta p_x$ 集中在正值侧 (ypol)；loose px100 的覆盖范围大得多。reco-level 的 truth$\to$reco 迁移是另一个问题，由 \S 4 的 response matrix 覆盖——本节看的是 cut 的几何选择，不是探测器 smearing。
...
当前画图所用的 QMD 样本 (Sn124，4 个 $\gamma$ 点求和) 在两个极化方向上极不均衡:
...
\midrule
ypol & $1.68\times10^6$ & $4.36\times10^4$ & $9.52\times10^3$ & 2.59\% & 0.566\% \\
zpol & $3.75\times10^4$ & $6.04\times10^3$ & $1.45\times10^3$ & 16.1\% & 3.87\% \\
\bottomrule
\end{tabular}
\end{table}

ypol 生成了 $1.68\times10^6$ 个 QMD 事例，zpol 只有 $3.75\times10^4$ 个 (相差 45 倍)。zpol 的通过率更高 (16.1\% vs 2.59\%)，但 raw 样本太小，导致当前 zpol 图的 MC 统计误差偏大 ($\sigma(R)_{\mathrm{sim}}\sim0.05$--$0.99$，见表 1)。$N_{\mathrm{reco}}$ 是当前 tight-px60 图实际使用的 usable $R$ 事件数。
...
\begin{tabular}{lrrrrr}
\toprule
pol & useful fraction & $N_{\mathrm{sim}}$ (当前图) & 16 h usable & 16 h $/$ $N_{\mathrm{sim}}$ & worst 3$\sigma$ $N_{\mathrm{req}}$ \\
\midrule
ypol & 0.00566 & $9.52\times10^3$ & $2.75\times10^5$ & $\sim$29$\times$ & $2.77\times10^3$ \\
zpol & 0.03869 & $1.45\times10^3$ & $1.88\times10^6$ & $\sim$1.3$\times10^3\times$ & $1.31\times10^2$ \\
\bottomrule
\end{tabular}
\end{table}

16 h 束流下 ypol 的 usable 统计是当前图的 $\sim$29 倍，zpol 是 $\sim$1300 倍。useful fraction $f_{\mathrm{useful}}=N_{\mathrm{reco}}/N_{\mathrm{raw}}$ 把 detected coincidences 折算到 usable $R$ 事件: $N_{\mathrm{usable}}=4.86\times10^7\times f_{\mathrm{useful}}$。zpol 的 $\sim$1300 倍增益尤其关键: 当前 zpol MC 严重欠采样 ($1.45\times10^3$ usable 事件)，实验会大幅改善其精度。
...
最难区分的间隔是 ypol g070$\to$g080 ($\bar R=5.42$, $|\Delta R|=0.85$, 需 $2.77\times10^3$ 事件)；zpol 最难是 g050$\to$g060 ($\bar R=4.74$, $|\Delta R|=3.27$, 需 131)。useful-event 外推链为
\[
...
\]
ypol $f_{\mathrm{useful}}=0.00566$ 给 $N_{\mathrm{usable}}=4.86\times10^7\times0.00566\approx2.75\times10^5$；zpol $f_{\mathrm{useful}}=0.0387$ 给 $1.88\times10^6$。可用量与所需量之比: ypol $\approx 99\times$ (约两个数量级)，zpol $\approx 1.4\times10^4\times$ (约四个数量级)。
