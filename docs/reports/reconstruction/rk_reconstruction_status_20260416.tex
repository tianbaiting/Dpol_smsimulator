...
\centering
\includegraphics[width=\textwidth]{samurai_geometry_xz.png}
\vspace{-0.3cm}
...
\textbf{Source CSVs} (all live under \texttt{build/test\_output/ensemble\_n50/}):
\begin{itemize}\setlength{\itemsep}{0pt}
\item \texttt{rk\_fixed\_target\_pdc\_only\_error\_analysis/track\_summary.csv} --- per-event Fisher/Laplace (744 rows)
\item \texttt{.../profile\_samples.csv} --- 32-event Profile LH intervals (px, py, pz, $|p|$)
\item \texttt{.../bayesian\_samples.csv} --- 64-event MCMC intervals ($|p|$ only in current analyzer)
...
\midrule
\multirow{3}{*}{$p_x$} & \multirow{3}{*}{$-33.1$} & \multirow{3}{*}{$6.21$}
  & Fisher  & 7.52 & 1.21 & 0.052 & [0.038, 0.071] \\
  & & & Laplace & 7.52 & 1.21 & 0.054 & [0.039, 0.072] \\
  & & & Profile & 6.44 & 1.04 & 0.000 & [0.000, 0.109] \\
\midrule
\multirow{3}{*}{$p_y$} & \multirow{3}{*}{$-0.04$} & \multirow{3}{*}{$1.40$}
  & Fisher  & 0.77 & 0.55 & 0.426 & [0.390, 0.463] \\
  & & & Laplace & 0.77 & 0.55 & 0.427 & [0.392, 0.464] \\
  & & & Profile & 0.56 & 0.40 & 0.344 & [0.186, 0.532] \\
\midrule
\multirow{3}{*}{$p_z$} & \multirow{3}{*}{$-1.49$} & \multirow{3}{*}{$6.60$}
  & Fisher  & 6.43 & 0.97 & 0.680 & [0.645, 0.714] \\
  & & & Laplace & 6.43 & 0.97 & 0.681 & [0.647, 0.715] \\
  & & & Profile & 5.45 & 0.83 & 0.625 & [0.437, 0.789] \\
\midrule
\multirow{4}{*}{$|\vec{p}|$} & \multirow{4}{*}{$-0.66$} & \multirow{4}{*}{$5.30$}
  & Fisher  & 6.68 & 1.26 & 0.761 & [0.728, 0.791] \\
  & & & Laplace & 6.68 & 1.26 & 0.769 & [0.737, 0.799] \\
  & & & Profile & 5.64 & 1.06 & 0.813 & [0.636, 0.928] \\
  & & & MCMC    & 5.16 & 0.97 & 0.656 & [0.527, 0.771] \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Takeaways} (with Sn target disabled): (i) $|\vec{p}|$ and $p_z$ half-widths are now \emph{well-matched} to realised residual spread (ratio $0.83$--$1.26$) and the observed 68\% coverage lands within its CP CI of 68\% --- Fisher actually works for the curvature-constrained directions; (ii) $p_y$ half-width is still under-reported by $\sim\!1.8\times$ (residual hw 1.4 vs.\ Fisher 0.77) $\Rightarrow$ coverage 43\% rather than 68\%; (iii) $p_x$ is a \emph{systematic bias problem}, not a coverage problem: half-widths match (residual hw 6.21 vs.\ Fisher 7.52, ratio 1.2), but the median residual is shifted by $-33$~MeV/$c$ so truth falls $\sim\!4.4\sigma$ below reco for every event $\Rightarrow$ coverage 5\%; (iv) MCMC per-component missing because current \texttt{analyze\_pdc\_rk\_error} samples only $|\vec p|$ in \texttt{bayesian\_samples.csv}.
\end{frame}
...
\textbf{Consequence}: all four share the \emph{same} forward model (RK4 trajectory + PDC residuals). A bias or linearization error in RK propagates into every interval. With the Sn target disabled, the $p_z$ residual std collapses and $|\vec p|$/$p_z$ coverage recovers to nominal 68\%; what remains is (a) a persistent $p_x$ median bias of $-33\;\mathrm{MeV}/c$ that drives $p_x$ coverage to 5\% even though the per-event spread matches Fisher (i.e.\ the RK forward model is mis-centred, not noisy) and (b) $(u,v,p)$ parameterization giving Fisher $\sigma_{p_y}\!\approx\!0.77\;\mathrm{MeV}/c$ while realised $p_y$ spread is 1.4 MeV/$c$ (ratio 1.8, coverage 43\%). Different wrappers around the same biased forward model cannot fix (a); fixing it needs $dE/dx$ in the propagator and a full Cartesian/unparameterised refit.
\end{frame}
...
$|\vec{p}|$ & MCMC    & 64  & 0.656 & [0.527, 0.771] & 0.891 & [0.788, 0.955] \\
$|\vec{p}|$ & Profile & 32  & 0.813 & [0.636, 0.928] & 0.969 & [0.838, 0.999] \\
\midrule
$p_x$ & Fisher  & 744 & \textbf{0.052} & [0.038, 0.071] & \textbf{0.196} & [0.168, 0.227] \\
$p_x$ & Laplace & 744 & 0.054 & [0.039, 0.072] & --    & --              \\
$p_x$ & Profile & 32  & \textbf{0.000} & [0.000, 0.109] & 0.000 & [0.000, 0.109] \\
\midrule
$p_y$ & Fisher  & 744 & 0.426 & [0.390, 0.463] & 0.691 & [0.656, 0.724] \\
$p_y$ & Laplace & 744 & 0.427 & [0.392, 0.464] & --    & --              \\
$p_y$ & Profile & 32  & 0.344 & [0.186, 0.532] & 0.562 & [0.377, 0.736] \\
\midrule
$p_z$ & Fisher  & 744 & 0.680 & [0.645, 0.714] & 0.870 & [0.843, 0.893] \\
$p_z$ & Laplace & 744 & 0.681 & [0.647, 0.715] & --    & --              \\
$p_z$ & Profile & 32  & 0.625 & [0.437, 0.789] & 0.906 & [0.750, 0.980] \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Verdict}. The picture splits cleanly by direction:
\begin{itemize}\setlength{\itemsep}{0pt}
\item \textbf{$|\vec p|$ and $p_z$ land at nominal 68\%} (Fisher $|\vec p|$ 76\%, $p_z$ 68\%) --- with the target MS removed, the dipole curvature constraint is tight enough that Fisher's Gaussian interval correctly tracks the realised spread. $|\vec p|$ is slightly over-conservative.
\item \textbf{$p_y$ mildly under-covers} (43\% at 68\% nominal). Residual half-width 1.40 vs.\ Fisher $\sigma$ 0.77 MeV/$c$ (1.8$\times$). Root cause: $(u,v,p)$ parameterisation still deflates $p_y$ Fisher info by $\sim\!2\times$, even without MS.
\item \textbf{$p_x$ is pathological} (5\%). Residual \emph{spread} matches Fisher (1.2$\times$), but median bias is $-33\;\mathrm{MeV}/c$ --- truth lies $\sim\!4.4\sigma$ below reco for \emph{every} event. This is reconstruction miscalibration, not a coverage miscount. Profile LH confirms (0/32 covered) since it shares the same biased forward model.
\end{itemize}
Drivers: missing $dE/dx$ (biases $p_z$ projection, and indirectly $p_x$ via coupled RK Jacobian) and $(u,v,p)$ linearisation (depresses $p_y$ Fisher).
\end{frame}
...
\item Output: \texttt{docs/reports/reconstruction/pdc\_hit\_dispersion\_px\{...\}\_py\{...\}.png}
\end{itemize}
...
\centering
\includegraphics[width=0.96\textwidth]{pdc_hit_dispersion_pxp0_pyp0.png}
...
\centering
\includegraphics[width=0.96\textwidth]{pdc_hit_dispersion_pxp100_pyp0.png}
...
\centering
\includegraphics[width=0.99\textwidth]{pdc_hit_dispersion_overview.png}
...
\textbf{Consequence}. When we now take the 50 same-truth replicas and look at how much the \emph{reconstructed} momentum scatters (histogram below), the global G4/Fisher halfwidth ratios are $p_x:\,0.83$, $p_y:\,1.82$, $p_z:\,1.03$, $|\vec p|:\,0.99$. Fisher is almost exactly right for $p_z/|\vec p|$, 1.8$\times$ too optimistic for $p_y$, and --- once you strip the per-event bias away --- also close for $p_x$. The dominant $p_x$ failure mode is a \emph{median bias} ($-33\;\mathrm{MeV}/c$, Step 2 baseline frame), not a mis-sized error bar.

\textbf{Source scripts}:
\begin{itemize}
\item \texttt{scripts/analysis/plot\_reco\_momentum\_dispersion.py}
\item Output: \texttt{docs/reports/reconstruction/reco\_momentum\_dispersion\_px\{...\}\_py\{...\}.png}
\end{itemize}
\end{frame}
...
\centering
\includegraphics[width=0.98\textwidth]{reco_momentum_dispersion_pxp0_pyp0.png}

\vspace{0.1cm}
\small
Black line = truth, red dashed = G4 median, pink band = Fisher $\sigma$ around median. $N{=}49$ replicas. $p_x$: median shifted to $\sim\!-32\;\mathrm{MeV}/c$, G4 $\sigma\!=\!4.7$ vs.\ Fisher $\sigma\!=\!7.2$, ratio 0.66 --- so the \emph{spread} is fine but the fitter's $p_x$ zero is wrong. $p_y$: ratio 1.9 (G4 1.5 vs.\ Fisher 0.8), the only direction where Fisher under-reports the realised spread. $p_z$: ratio 0.71 (G4 4.1 vs.\ Fisher 5.8), essentially matched. A handful of outliers near $|p|\!\sim\!0$ or double-peaked failures still appear but are numerically small.
\end{frame}

% ============================================================
\begin{frame}{Reco Momentum Dispersion --- $(+100,0,627)$}
\centering
\includegraphics[width=0.98\textwidth]{reco_momentum_dispersion_pxp100_pyp0.png}

\vspace{0.1cm}
\small
$N{=}50$. $p_x$: G4 $\sigma\!=\!4.0$ vs.\ Fisher $\sigma\!=\!6.9$, ratio 0.58 (Fisher conservative); the median is much closer to truth than at $(-100,0)$ --- biases are strongly direction-dependent. $p_y$: ratio 2.8 (G4 1.4 vs.\ Fisher 0.5) --- Fisher here is the tightest $\sigma$ in the grid yet the realised spread is typical, so the mismatch is largest at this point. $p_z$: ratio 0.70 (G4 5.1 vs.\ Fisher 7.3). Point-by-point heterogeneity is real --- the ratios vary by $\sim 2$--$3\times$ across the 15 truth points and are summarised on the next slide.
\end{frame}
...
\midrule
$0$    & $0$   & 2.8  & 11.2 & 3.5  & 15.2 & 0.66 & 1.87 & 0.71 \\
$+100$ & $0$   & 3.7  &  9.1 & 5.7  & 12.3 & 0.58 & 2.80 & 0.70 \\
$-100$ & $0$   & 2.7  & 10.7 & 3.0  & 14.3 & 3.62 & 1.65 & 3.64 \\
$0$    & $+20$ & 2.9  &  9.6 & 4.0  & 12.6 & 0.92 & 1.88 & 0.74 \\
$0$    & $-20$ & 3.2  &  8.4 & 4.6  & 10.6 & 0.86 & 1.52 & 0.75 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Read-across}:
\begin{itemize}
  \item Without the Sn target the PDC $\sigma_y$ collapses from ${\sim}100$~mm to ${\sim}10$--$15$~mm and the reco-momentum G4/Fisher ratios land near 1 for $p_x,p_z$ and ${\sim}2$ for $p_y$ --- Fisher \emph{sizing} is essentially correct everywhere except $p_y$.
  \item $(-100, 0)$ is the one point where $p_x/p_z$ G4 halfwidths blow up to $\sim\!36$ and $\sim\!21$ MeV/$c$ (ratios 3.6). This is the LM coarse-grid coverage hole at the PDC acceptance edge (inward-bending trajectories with marginal geometry); see coverage findings for RK at that grid point in the project memo.
  \item The remaining coverage shortfall (5\% on $p_x$, 43\% on $p_y$) is now clearly separated into two causes: (a) residual $p_x$ bias of order ${-30}$~MeV/$c$ that Fisher \emph{cannot} correct (error bar is the wrong object); (b) $p_y$ Fisher-sigma under-sizing by $\sim 2\times$ driven by $(u,v,p)$ linearisation. Adding $dE/dx$ + Cartesian refit is the natural fix for both.
\end{itemize}
\end{frame}
...
\centering
\includegraphics[width=0.99\textwidth]{reco_momentum_dispersion_grid_px.png}

\vspace{0.05cm}
{\scriptsize Rows = truth $p_y \in \{+20, 0, -20\}$, cols = truth $p_x \in \{-100,-50,0,50,100\}$ MeV/$c$. Per cell: histogram of $N{=}48$--$50$ reco $p_x$ values, black = truth, red dashed = G4 median, pink = Fisher $\sigma$ band. Annotated $r$ = G4 $\sigma$ / Fisher $\sigma$. Bias is typically $-20$ to $-40\;\mathrm{MeV}/c$ and flips sign near $(-100,0)$ where the LM solver hits a coarse-grid basin; $\sigma$ itself tracks Fisher within a factor of 2 except at that point.}
\end{frame}

% ============================================================
\begin{frame}{Reco $p_y$ Dispersion --- Full $5\times 3$ Truth Grid}
\centering
\includegraphics[width=0.99\textwidth]{reco_momentum_dispersion_grid_py.png}

\vspace{0.05cm}
{\scriptsize Same layout as previous slide, now for $p_y$. The realised G4 halfwidth is $\sim 1$--$2\;\mathrm{MeV}/c$ across the grid; Fisher $\sigma_{p_y}$ is consistently $0.5$--$1.0\;\mathrm{MeV}/c$, giving ratios of $1.5$--$2.8$. This is the systematic Fisher-sigma under-sizing discussed in the Outlook --- the $(u,v,p)$ parameterisation linearises the $p_y$ response in a way that excludes a real fraction of the injected spread.}
\end{frame}

% ============================================================
\begin{frame}{Reco $p_z$ Dispersion --- Full $5\times 3$ Truth Grid}
\centering
\includegraphics[width=0.99\textwidth]{reco_momentum_dispersion_grid_pz.png}

\vspace{0.05cm}
{\scriptsize Same layout, now for $p_z$. G4/Fisher ratios sit at $0.7$--$0.8$ almost everywhere --- Fisher is slightly conservative for $p_z$, consistent with the global ratio of $1.03$ from the residual analysis. The $(-100,0)$ cell is again the outlier, with G4 $\sigma\!\sim\!21\;\mathrm{MeV}/c$ driven by LM failures at the PDC acceptance edge. Truth $p_z=627\;\mathrm{MeV}/c$ throughout.}
\end{frame}
...
\item \textbf{Fix under-coverage}: the 744-event target-off ensemble (this report) shows Fisher 68\% coverage $= 76\%$ ($|\vec p|$), $68\%$ ($p_z$), $43\%$ ($p_y$), $5\%$ ($p_x$). $p_z/|\vec p|$ are already at nominal; the two outstanding issues are (i) the $p_x$ median bias of $-33\;\mathrm{MeV}/c$ --- needs a Cartesian refit (no $(u,v,p)$ linearisation) and $dE/dx$ in the RK propagator; (ii) the $p_y$ Fisher $\sigma$ deflation by $\sim 2\times$, same root cause. Retest on the 89k-event dataset once the fit is Cartesian.
