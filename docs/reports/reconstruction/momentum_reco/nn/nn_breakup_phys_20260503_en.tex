...
\begin{itemize}[nosep]
    ...
    \item \texttt{NEBULAReconstructor::ApplyPositionSmearing} adds a 5 mm Gaussian which is far smaller than the 12 cm bar pitch, so the discreteness is essentially preserved. $12$ cm / $L_\text{NEBULA}=7.25$ m $\times |\vec p|\approx 600$ MeV/$c$ $\to$ a rectangle of $\sim\pm 5$ MeV/$c$ in $\Delta p_x$, consistent with what is observed.
