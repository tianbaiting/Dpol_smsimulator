% Build: cd docs/reports/reconstruction/momentum_reco/rk && latexmk -xelatex rk_reconstruction_status_20260416.tex
...
% Path to the analysis run that produced all numerical results in
...
% ============================================================
\begin{frame}{Run Configuration for Numerical Results}
\small
Subsequent frames in this section quote numbers from two complementary runs.
The default RK fit mode is \texttt{kThreePointFree} (5 free params $(\delta x, \delta y, u, v, p)$ with a soft target prior $\sigma_{xy}^\mathrm{tgt} = 5\;\mathrm{mm}$); all examples here use \texttt{kFixedTargetPdcOnly} ($\delta x, \delta y$ fixed at 0; only $(u, v, p)$ free), which matches the dataset prefixes shown below.
...
Source & Path & Use \\
\midrule
Smoke (5 events) & \texttt{build/test\_output/error\_analysis\_rk/} & Per-event Fisher table (frame ``Error Analysis Results'') \\
Ensemble (744 events) & \texttt{build/test\_output/ensemble\_n50\_targetframe/rk\_fixed\_target\_pdc\_only\_error\_analysis/} & Coverage validation across truth grid \\
Realistic legacy (815 tracks) & \texttt{results/pdc\_rk\_error\_after\_pdcana\_20260411/} & Companion zh.\ report: per-track Fisher/Laplace + 32 Profile + 16 MCMC; no truth $\Rightarrow$ no coverage \\
\bottomrule
\end{tabular}

\vspace{0.15cm}
The Chinese companion (\texttt{rk\_reconstruction\_status\_20260416\_zh.pdf}) contains a worked example, the per-method comparison plots from the realistic-data run, and a discussion of why MCMC is broader than Profile in that run.
\end{frame}
...
% ============================================================
\begin{frame}{Fisher: Algorithm Outline and Cost}
\small
Per-event computation by \texttt{PDCErrorAnalysis::computeFisher} (\texttt{libs/analysis\_pdc\_reco/src/PDCErrorAnalysis.cc}):

\begin{enumerate}\setlength{\itemsep}{0pt}
\item \textbf{Jacobian} $J = \partial \vec{w}/\partial \vec{\alpha}$ via central differences;
$2 n_\mathrm{params}$ RK trajectory propagations, step
$h_u = \max(10^{-4}, 0.01|u|)$, $h_p = \max(0.5\;\mathrm{MeV}/c,\,10^{-3}p)$.
\item \textbf{Information} $\mathcal{I} = J^T J$ ($3{\times}3$ for \texttt{kFixedTargetPdcOnly}, $5{\times}5$ for \texttt{kThreePointFree}).
\item \textbf{SVD} $\mathcal{I} = U S V^T$; report condition $\kappa = S_\mathrm{max}/S_\mathrm{min}$, fail if $\kappa$ exceeds threshold.
\item \textbf{Param covariance} $\Sigma_{\vec{\alpha}} = V S^{-1} U^T$.
\item \textbf{Momentum Jacobian} $M = \partial(p_x,p_y,p_z)/\partial\vec{\alpha}$ (central differences).
\item \textbf{Momentum covariance} $\Sigma_{\vec{p}} = M\,\Sigma_{\vec{\alpha}}\,M^T$.
\end{enumerate}

\textbf{Cost:} 1 Jacobian eval ($\sim\!6$ RK propagations) + 1 SVD $\approx 5\;\mathrm{ms}$ per event $\Rightarrow$ runs inline.

\vspace{0.1cm}
\textbf{Worked example} (event 0 from the smoke run, MLE point estimate
$(p_x, p_y, p_z) = (-107.33,\, -13.35,\, 610.90)$~MeV/$c$):
\[
\sigma_{p_x} = 3.83,\ \sigma_{p_y} = 0.15,\ \sigma_{p_z} = 2.22\ \text{MeV}/c,\quad \chi^2_\mathrm{red} = 0.18.
\]
The order-of-magnitude difference between $\sigma_{p_y} \sim 0.15$ and $\sigma_{p_x},\sigma_{p_z} \sim 2\text{--}4$~MeV/$c$ is the geometric Jacobian effect: the wire direction has its largest projection on $\hat{y}$ ($\pm 1/\sqrt{2}$), so the wire residual most strongly constrains $v = p_y/p_z$, shrinking $\sigma_{p_y}$ in $(J^T J)^{-1}$. Same pattern persists across the 744-event ensemble (see ``Ensemble Coverage'' frame).
\end{frame}
...
% ============================================================
\begin{frame}{MC Propagation: Algorithm and Cost}
\small
Implemented at \texttt{libs/analysis\_pdc\_reco/src/PDCRkAnalysisInternal.cc:850}.

\begin{enumerate}\setlength{\itemsep}{0pt}
\item Check $\Sigma_{\vec{\alpha}}$ PSD; if not, fall back to Jacobian linear propagation.
\item $\Sigma_{\vec{\alpha}}^{1/2} \gets$ \texttt{SymmetricSqrt}($\Sigma_{\vec{\alpha}}$).
\item Loop $s = 1 \dots 256$: $\vec{z} \sim \mathcal{N}(0, I)$;
$\vec{\alpha}^{(s)} \gets \mathrm{ClampState}(\hat{\vec{\alpha}} + \Sigma^{1/2} \vec{z})$;
$\vec{p}^{(s)} \gets \mathrm{BuildMomentumVector}(\vec{\alpha}^{(s)})$ (pure algebra).
\item Empirical $q_{16/84}$, $q_{2.5/97.5}$ for $p_x$, $p_y$, $p_z$, $|\vec{p}|$. RNG seed fixed (\texttt{0xC0FFEE}).
\end{enumerate}

\textbf{Cost}: 256 algebraic transforms, $< 1$ ms per event.

\vspace{0.1cm}
\textcolor{red}{\textbf{Implementation note}}: when state covariance is PSD, this routine's outputs are written into the \emph{Laplace} \texttt{IntervalEstimate} slots in \texttt{track\_summary.csv}, not into a separate ``MC propagation'' column. So the CSV's \texttt{laplace\_*\_lower68 / upper68} columns actually carry the 256-sample empirical quantiles, not Gaussian projections of $\Sigma_\mathrm{Laplace}$.
\end{frame}
...
% ============================================================
\begin{frame}{Profile LH: Algorithm and Cost}
\small
\texttt{PDCErrorAnalysis::ComputeProfileLikelihood} returns \texttt{std::array<ProfileLikelihoodResult, 5>} (one slot per parameter $\delta x, \delta y, u, v, p$; inactive slots have \texttt{valid = false}).

\textbf{Per-parameter scan}:
\begin{enumerate}\setlength{\itemsep}{0pt}
\item Range $[\hat{\alpha}_k - 2.5\sigma_{\alpha_k},\; \hat{\alpha}_k + 2.5\sigma_{\alpha_k}]$, $N_\mathrm{scan} = 17$ equispaced points.
\item Warm-start: $\vec{\alpha}_\mathrm{warm} \gets \hat{\vec{\alpha}}$.
\item For each scan point: fix $\alpha_k = \alpha_k^{(i)}$; LM-optimize the remaining params from $\vec{\alpha}_\mathrm{warm}$; record $\chi^2_\mathrm{profile}(\alpha_k^{(i)})$; $\vec{\alpha}_\mathrm{warm} \gets$ new optimum (warm-start chains adjacent points).
\item Linear interpolation for $\Delta\chi^2 = 1.0$ (68\%) and $\Delta\chi^2 = 4.0$ (95\%) crossings.
\end{enumerate}

\textbf{Cost}: $\sim$20 LM optimizations per parameter, $\sim$10 ms each $\Rightarrow$ $\sim 0.6$ s per scanned parameter. Practical for offline diagnostic, not per-event runtime.

\vspace{0.1cm}
\textbf{Worked example} (event 399 from realistic legacy run, $|\vec p|$):
$\hat{p} = 667.75$, Profile 68\% $= [660.73,\, 667.77]$ MeV/$c$ $\Rightarrow$ asymmetric: $\hat{p} - q^- = 7.02$, $q^+ - \hat{p} = 0.02$. Fisher gives symmetric $\pm 2.84$. Profile correctly flags that the $\chi^2$ rises sharply on the high-$p$ side (already $\chi^2_\mathrm{red} = 20.4$) but has a deeper basin on the low-$p$ side.
\end{frame}
...
% ============================================================
\begin{frame}{Laplace: Algorithm and Cost}
\small
\texttt{PDCErrorAnalysis::ComputeBayesianPosterior} (also drives MCMC); prior config from \texttt{BayesianConfig::momentum\_prior}.

\begin{enumerate}\setlength{\itemsep}{0pt}
\item $\Lambda_\mathrm{prior} \gets \mathrm{diag}(0,\dots,1/\sigma_\mathrm{prior}^2)$ if momentum prior enabled, else $\mathbf{0}$.
\item $\mathcal{H}_\mathrm{post} \gets J^T J + \Lambda_\mathrm{prior}$.
\item $\Sigma_\mathrm{post} \gets$ \texttt{SVDInverse}($\mathcal{H}_\mathrm{post}$) (same routine as Fisher, with conditioning check).
\item If $\Sigma_\mathrm{post}$ is PSD, dispatch to method-2's 256-sample MC for empirical quantiles. Else fall back to $M \Sigma_\mathrm{post} M^T$ + Gaussian symmetric intervals.
\end{enumerate}

\textbf{Cost}: 1 matrix add + 1 SVD inverse, $< 1$ ms.

\vspace{0.1cm}
\textbf{Worked example} (event 399, prior \emph{disabled} in this run): $\Lambda_\mathrm{prior} = \mathbf{0}$, so mathematically Laplace $\equiv$ Fisher. CSV shows Laplace 68\% $|\vec p|$ $= [664.91,\,670.58]$ (half-width $\approx 2.84$) vs.\ Fisher $\sigma_p = 2.84$ MeV/$c$ --- match exact. The asymmetry expected from method-2 dispatch is below the resolution of this event because $|\vec p|$ is low ($\sim 668$ MeV/$c$, near-linear regime).
\end{frame}

% ============================================================
\begin{frame}{MCMC: Algorithm and Cost}
\small
Same \texttt{ComputeBayesianPosterior}; convergence diagnostics in \texttt{ComputeGewekeZScore}, \texttt{ComputeEffectiveSampleSize}.

\textbf{Per-track loop} ($N_b = 5000$ burn-in $+ N_s = 50000$ samples, thin = 5 $\Rightarrow \sim 10000$ kept):
\begin{enumerate}\setlength{\itemsep}{0pt}
\item Each step: $\vec{\alpha}' \gets \vec{\alpha}^{(t-1)} + s \Sigma_\mathrm{post}^{1/2} \vec{z}$; $\log\pi'$ via 1 RK propagation; accept w.p.\ $\min(1, e^{\Delta\log\pi})$.
\item During burn-in, every 50 steps: rescale $s$ ($\times 1.1$ if accept rate $> 23\%$, else $\times 0.9$).
\item Post-burn: thin and store. Compute Geweke $z$ (first 10\% vs.\ last 50\% mean diff) and ESS (autocorrelation sum truncated at $|\hat\rho_k| < 0.05$).
\item \texttt{converged} flag $=$ (\texttt{ESS} $\geq 50$ \textbf{and} $|z| < 2$).
\end{enumerate}

\textbf{Cost}: $\sim$55 000 RK propagations $\times$ 0.1 ms $= \sim 5.5$ s per event.

\vspace{0.1cm}
\textbf{Worked example} (event 399 from realistic-legacy run; \texttt{bayesian\_samples.csv}):
MCMC 68\% $|\vec p| = [643.19, 654.27]$, width $11.1$ MeV/$c$ (centred $\sim 19$ MeV/$c$ below the LM optimum $667.75$). Acceptance $0.593$ (above MH-optimal $0.234$ $\Rightarrow$ proposal too small), ESS $4.28$, Geweke $|z| = 4.17$, \texttt{converged = 0}. \textbf{Chain unreliable} on this event --- the diagnostics correctly flag it. Remediation: longer burn-in / larger initial scale / NUTS.
\end{frame}

% ============================================================
\begin{frame}{MC PDC End-to-End: Algorithm and Cost}
\small
\texttt{PDCErrorAnalysis::RunMonteCarlo} + \texttt{PerturbTrack}; error model from \texttt{PDCErrorModel}.

\textbf{Per-track loop} ($N = 256$ samples, fitter $=$ same \texttt{PDCMomentumReconstructor} as central):
\begin{enumerate}\setlength{\itemsep}{0pt}
\item For each $k$: $(\delta u_1, \delta v_1)$, $(\delta u_2, \delta v_2) \sim \mathcal{N}(\vec 0, \Sigma_\mathrm{wire})$; perturb both PDC hits along $\hat u, \hat v$.
\item Rerun \emph{full} LM/RK4 reconstruction starting from $\hat{\vec{\alpha}}$, get $\vec p^{(k)}$.
\item Trim 5\% on each side of $|\vec p|$ to drop runs that converged to a wrong local minimum.
\item Empirical quantiles + sample covariance.
\end{enumerate}

\textbf{Cost}: $\sim 256 \times 100$ ms $= \sim 25$ s per event $\Rightarrow$ offline only ($\sim$50-event subsample).

\vspace{0.1cm}
\textbf{What's different from method-2 (param-space MC)}: this propagates from \emph{measurement} space ($\sigma_u, \sigma_v$ on PDC hits) through the entire fitter, so it sees fitter pathologies (warm-start sensitivity, multiple minima). Method-2 only propagates the parameter-space $\Sigma_{\vec{\alpha}}$ algebraically and never re-runs the fitter.

\vspace{0.05cm}
\textbf{Status in this study}: \emph{not enabled} in any of the three datasets summarized in the Run Configuration frame; reserved for offline studies of detector-resolution sensitivity.
\end{frame}
...
\begin{frame}{E2E Test Results: RK vs NN \textcolor{red}{[NN reference contaminated]}}
\small
\textcolor{red}{\textbf{Audit note (2026-04-20):} NN bias columns below were produced
by the now-deleted polluted model \texttt{formal\_B115T3deg\_qmdwindow/20260227\_223007/}
(target-contamination audit: \texttt{docs/audit/2026-04-20-nn\_target\_contamination\_audit.md}).
Clean retrain has val $p_x$ RMSE $-51\%$, $p_y$ $-81\%$, $p_z$ essentially unchanged ---
so the qualitative $p_z$ comparison (RK $-10.8$ vs NN $-3$ MeV/$c$, $dE/dx$-dominated) still
holds, but $p_x,p_y$ ``comparable'' conclusion is invalidated. Re-run
\texttt{test\_rk\_nn\_comparison.py} with
\texttt{formal\_B115T3deg/20260420\_184345/model/model.pt} before requoting.}
...
\begin{itemize}
\item $p_x$/$p_y$ biases are comparable between RK and NN here, \textcolor{red}{but the NN side is the polluted model --- clean NN is expected to be meaningfully better on $p_x,p_y$}.
\item $p_z$: RK shows a systematic negative bias ($-5$ to $-16$ MeV/$c$), likely from missing $dE/dx$. NN $p_z$ is nearly unchanged between polluted and clean models.
\end{itemize}
\end{frame}

% ============================================================
\begin{frame}{Three RK Modes --- Unified Performance \textcolor{red}{[NN row contaminated]}}
\small
...
\midrule
NN (reference, \textcolor{red}{polluted model}) & 21.0 & 15.6 & 3.1 \\
\bottomrule
\end{tabular}
\end{table}

\vspace{0.2cm}
\textbf{Before the bug fixes}, the RK modes had mean biases of 180--650 MeV/$c$ on $p_x$ and 190--605 MeV/$c$ on $p_z$. The improvement is $\mathbf{10\text{--}80\times}$. \textcolor{red}{NN reference row pending re-eval with clean model; RK numbers are geometry-only and unaffected.}
\end{frame}
...
{\footnotesize NN/RK MAE from \texttt{test\_rk\_nn\_comparison.py} on 5 E2E grid events. RK $p_z$ bias ($-10.8$~MeV/$c$ mean) attributed to missing $dE/dx$. \textcolor{red}{NN $p_z$ from polluted model; however target contamination perturbed $p_x,p_y$ much more than $p_z$, so $p_z$ comparison stays qualitatively intact.}}
\end{frame}
...
\item \textbf{Performance} (from E2E test): RK MAE = (20.4, 16.0, 10.8) MeV/$c$ for $(p_x, p_y, p_z)$; NN MAE = (21.0, 15.6, 3.1) MeV/$c$ \textcolor{red}{[NN from polluted model, see audit 2026-04-20; clean NN much better on $p_x,p_y$, similar on $p_z$]}. Fisher $\sigma_{|\vec{p}|}$ = 2.4--6.0 MeV/$c$.
\end{enumerate}
\end{frame}
