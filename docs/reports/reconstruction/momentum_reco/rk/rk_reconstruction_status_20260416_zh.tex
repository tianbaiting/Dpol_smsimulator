% Build: cd docs/reports/reconstruction/momentum_reco/rk && latexmk -xelatex rk_reconstruction_status_20260416_zh.tex
...
% [EN] Path to the analysis run that produced all numerical results in §误差分析.
% [CN] §误差分析 全部数值与图来自此 run。
\newcommand{\AnaRoot}{../../../../../results/pdc_rk_error_after_pdcana_20260411}
...
\paragraph{本节范围与数据来源说明。}
本节后续\textbf{所有数值结果与对比图}均来自分析 run \texttt{pdc\_rk\_error\_after\_pdcana\_20260411}（共 815 条成功重建轨迹），由 \texttt{apps/tools/analyze\_pdc\_rk\_error.cc} 在默认 RK 拟合模式 \texttt{kThreePointFree} 下产出。该模式以 5 个自由参数 $(\delta x, \delta y, u, v, p)$ 拟合，靶点 $(\delta x, \delta y)$ 通过软先验（$\sigma_{xy}^\mathrm{tgt}=5\;\mathrm{mm}$）约束。下文方法一（Fisher）的数学推导沿用 \texttt{kFixedTargetPdcOnly} 的 3 参数 $(u,v,p)$ 形式以保持代数简洁——结果的 $(u,v,p)$ 子块同样适用于 \texttt{kThreePointFree} 拟合，差别仅是 Fisher 信息矩阵从 $3{\times}3$ 扩展为 $5{\times}5$ 并多出 $\delta x,\delta y$ 子块。Worked example（\S\ref{sec:fisher} 末尾）的数字直接来自 \texttt{kThreePointFree} 输出。

\paragraph{覆盖率验证缺位。}
此 run 的 \texttt{after\_pdcana} 数据缺失 \texttt{OriginalParticleData} truth 字典，故 \texttt{validation\_summary.csv} 中四种方法的 \texttt{cover68/95} 全为 0——\textbf{本 run 给不出覆盖率检验}。覆盖率结论另见 2026-04-17 的 ensemble study（独立的 truth-grid run）：在该 study 中 Fisher、MCMC、Profile 在 68\% 处全部欠覆盖，主要系统误差来自未建模的 $dE/dx$ 与 $(u,v,p)$ 局部线性化。
...
\paragraph{完整算法流程。}
为消除"理论很抽象"的疑虑，下面给出 \texttt{PDCErrorAnalysis::computeFisher} 的完整算法骨架，每一步都对应代码中的具体函数：

\begin{algorithm}[H]
\caption{Fisher 协方差计算（每条 track 调用一次）}
\label{alg:fisher}
\begin{algorithmic}[1]
\Require LM 收敛后的最优参数 $\hat{\vec{\alpha}}$、白化因子 $L$（Cholesky）、磁场地图、PDC 几何
\Ensure 参数协方差 $\Sigma_{\vec{\alpha}}$、动量协方差 $\Sigma_{\vec{p}}$、条件数 $\kappa$
\State $J \gets$ \Call{ComputeWhitenedResidualJacobian}{$\hat{\vec{\alpha}}, L$} \Comment{$2 n_\mathrm{params}$ 次 RK 传播，中心差分}
\State $\mathcal{I} \gets J^T J$ \Comment{$3{\times}3$ 或 $5{\times}5$ 取决于 RK 模式}
\State $(U, S, V^T), \kappa \gets$ \Call{SVD}{$\mathcal{I}$}
\If{$\kappa > \kappa_\mathrm{max}$ \textbf{或} $\min(S) < \epsilon$}
  \State \textbf{return} 失败 \Comment{病态 Fisher}
\EndIf
\State $\Sigma_{\vec{\alpha}} \gets V S^{-1} U^T$
\State $M \gets \partial(p_x, p_y, p_z) / \partial \vec{\alpha}$ \Comment{中心差分；$3 {\times} n_\mathrm{params}$}
\State $\Sigma_{\vec{p}} \gets M\, \Sigma_{\vec{\alpha}}\, M^T$
\State \textbf{return} $(\Sigma_{\vec{\alpha}}, \Sigma_{\vec{p}}, \kappa)$
\end{algorithmic}
\end{algorithm}

代码锚点：
\begin{itemize}[nosep]
  \item \texttt{libs/analysis\_pdc\_reco/include/PDCErrorAnalysis.hh}：接口、\texttt{IntervalEstimate} 结构。
  \item \texttt{libs/analysis\_pdc\_reco/src/PDCErrorAnalysis.cc}：\texttt{computeFisher}/\texttt{computeMomentumCovariance}。
  \item \texttt{libs/analysis\_pdc\_reco/src/PDCRkAnalysisInternal.cc}：白化残差、Jacobian 数值差分（与 LM 内部使用同一例程）。
  \item \texttt{libs/analysis\_pdc\_reco/include/PDCRecoTypes.hh}：\texttt{RkFitMode} 枚举与参数计数 \texttt{kParameterCount}。
\end{itemize}

\paragraph{Worked example：event \#399，reco $|\vec{p}| \approx 668\;\mathrm{MeV}/c$。}
为让读者把抽象的 $J$、$\Sigma$ 与具体数字挂上钩，下表摘录该事件的 \texttt{kThreePointFree} 拟合输出（\texttt{track\_summary.csv}，源文件 \texttt{Pb208\_g050\_ypol\_np\_Pb208\_g0500000\_reco.root}）：

\begin{table}[H]
\centering
\caption{Event \#399 Fisher 输出（kThreePointFree 模式，9 次 LM 迭代收敛）}
\label{tab:fisher_event399}
\begin{tabular}{lccc}
\toprule
量 & $p_x$ & $p_y$ & $p_z$ \\
\midrule
重建中心值 (MeV/$c$) & $-206.49$ & $-54.60$ & $632.67$ \\
Fisher $\sigma$ (MeV/$c$) & $2.26$ & $0.35$ & $2.25$ \\
Laplace 68\% 区间 (MeV/$c$) & $[-208.75,\,-204.23]$ & $[-54.95,\,-54.25]$ & $[630.42,\,634.91]$ \\
Fisher 半宽 / Laplace 半宽 & $1.00$ & $1.00$ & $1.00$ \\
\bottomrule
\end{tabular}
\end{table}
...
可以看出：
\begin{itemize}[nosep]
  \item $\sigma_{p_y} \approx 0.35\;\mathrm{MeV}/c$，比 $\sigma_{p_x},\sigma_{p_z} \approx 2.3\;\mathrm{MeV}/c$ 小约一个量级——与前文"丝方向 $y$ 分量最大、$v=p_y/p_z$ 被强约束"的论断完全一致。
  \item Fisher 与 Laplace 的半宽几乎完全相等（比值 = 1.00），说明在此事件上动量先验对后验的贡献可以忽略，二者均退化为局部高斯近似（$|\vec{p}|$ 上的 \texttt{summary.csv} 全 run median 也呈现同样规律：48.20 vs 48.14 MeV/$c$）。
  \item $\chi^2_\mathrm{red} = 20.4$（\texttt{ndf}$=3$）远大于 1，提示模型与数据不完全相容（很可能是未建模的 $dE/dx$ 偏置）。Fisher 给出的 $\sigma$ 在此情况下\textbf{偏小}——对照后文 §\ref{sec:profile} 的 Profile 区间和 §\ref{sec:mcmc} 的 MCMC 区间可以验证这一点。
\end{itemize}
...
\begin{algorithm}[H]
\caption{方法二：参数空间 256 点蒙特卡洛传播}
\label{alg:mc_prop}
\begin{algorithmic}[1]
\Require LM 收敛态 $\hat{\vec{\alpha}}$、状态协方差 $\Sigma_{\vec{\alpha}}$（由 Fisher 提供）
\Ensure $(p_x, p_y, p_z, |\vec{p}|)$ 各自的 68\%/95\% 经验分位区间
\State 检查 $\Sigma_{\vec{\alpha}}$ 是否半正定；若否则退化为 Jacobian 线性传播并 \textbf{return}
\State $\Sigma^{1/2} \gets$ \Call{SymmetricSqrt}{$\Sigma_{\vec{\alpha}}$} \Comment{特征分解构造对称平方根}
\For{$s = 1$ \textbf{to} $256$}
  \State $\vec{z} \sim \mathcal{N}(\vec{0}, I_{n \times n})$
  \State $\vec{\alpha}^{(s)} \gets$ \Call{ClampState}{$\hat{\vec{\alpha}} + \Sigma^{1/2} \vec{z}$}
  \State $\vec{p}^{(s)} \gets$ \Call{BuildMomentumVector}{$\vec{\alpha}^{(s)}$} \Comment{纯代数变换；零次 RK 传播}
\EndFor
\State 取 $\{p_x^{(s)}\}, \{p_y^{(s)}\}, \{p_z^{(s)}\}, \{|\vec{p}|^{(s)}\}$ 的经验分位数 $q_{16/84}$、$q_{2.5/97.5}$
\State \textbf{return} 经验区间（若协方差非 PSD 则线性后备）
\end{algorithmic}
\end{algorithm}

代码锚点：\texttt{libs/analysis\_pdc\_reco/src/PDCRkAnalysisInternal.cc:850}（无名 lambda \texttt{fill\_from\_jacobian} + 256 点循环）；
RNG 种子固定为 \texttt{0xC0FFEE} 以保证结果可复现；样本封装到 \texttt{IntervalEstimate} 写入 \texttt{BayesianResult}（与 Laplace 共享同一组输出 slot）。
...
\paragraph{在本 run 中的实际身份。}
\textbf{重要}：在 \texttt{analyze\_pdc\_rk\_error} 的实现中，方法二的 256 点 MC 输出与 Laplace 后验复用同一组 \texttt{IntervalEstimate} slot——只要状态协方差是 PSD，CSV 中标记为 \texttt{laplace\_*\_lower68}/\texttt{upper68} 的列就是\textbf{方法二的经验分位数}（非线性 MC），而非纯 Laplace 解析高斯。换言之，本 run 的"Laplace 列"已经包含了 $(u,v,p) \to (p_x,p_y,p_z)$ 的非线性效应。这解释了为什么对 event \#399 的 Laplace 区间几乎与 Fisher 半宽精确相等（$\sigma$-projection 的对称区间），但对其他事件（特别是大 $p$、强非线性区域）会出现轻度非对称。
...
\begin{algorithm}[H]
\caption{方法三：Profile Likelihood 扫描（针对参数 $\alpha_k$）}
\label{alg:profile}
\begin{algorithmic}[1]
\Require LM 中心点 $\hat{\vec{\alpha}}$、Fisher $\sigma_{\alpha_k}$、扫描点数 $N_\mathrm{scan}$（默认 17）
\Ensure 68\% 与 95\% Profile 置信区间 $[\alpha_k^{lo}, \alpha_k^{hi}]$
\State 扫描范围 $[\hat{\alpha}_k - 2.5\sigma_{\alpha_k},\; \hat{\alpha}_k + 2.5\sigma_{\alpha_k}]$ 取 $N_\mathrm{scan}$ 等距点
\State $\vec{\alpha}_\mathrm{warm} \gets \hat{\vec{\alpha}}$ \Comment{warm-start 起点}
\For{每个扫描点 $\alpha_k^{(i)}$}
  \State 固定 $\alpha_k = \alpha_k^{(i)}$，从 $\vec{\alpha}_\mathrm{warm}$ 起对 $\alpha_{j \neq k}$ 跑 LM 优化（共享 RK4 + Jacobian 例程）
  \State 记录 $\chi^2_\mathrm{profile}(\alpha_k^{(i)})$；$\vec{\alpha}_\mathrm{warm} \gets$ 本步收敛态
\EndFor
\State $\Delta\chi^2(\alpha_k^{(i)}) \gets \chi^2_\mathrm{profile} - \chi^2_\mathrm{min}$
\State 在相邻扫描点间线性插值，求 $\Delta\chi^2 = 1.0$（68\%）与 $\Delta\chi^2 = 4.0$（95\%）的两个交叉点
\State \textbf{return} 交叉点构成的非对称区间
\end{algorithmic}
\end{algorithm}

代码锚点：\texttt{PDCErrorAnalysis::ComputeProfileLikelihood} 返回 \texttt{std::array<ProfileLikelihoodResult, 5>}（5 个槽位对应 $\delta x, \delta y, u, v, p$，未启用的参数返回 \texttt{valid=false}）；扫描参数由 \texttt{ProfileLikelihoodConfig} 控制；CSV 写入由 \texttt{analyze\_pdc\_rk\_error.cc} 完成。

\paragraph{Worked example: event \#399。}
\texttt{profile\_samples.csv} 给出该事件的 Profile 区间：
\begin{table}[H]
\centering
\caption{Event \#399 Profile vs.\ Fisher/Laplace 68\% 区间对比}
\label{tab:profile_event399}
\begin{tabular}{lccc}
\toprule
量 & 中心 (MeV/$c$) & 区间下限 & 区间上限 \\
\midrule
$p_x$（Profile） & $-206.49$ & $-206.50$ & $-200.81$ \\
$p_x$（Fisher $\pm \sigma$） & $-206.49$ & $-208.75$ & $-204.23$ \\
$|\vec{p}|$（Profile） & $667.75$ & $660.73$ & $667.77$ \\
$|\vec{p}|$（Fisher $\pm \sigma$） & $667.75$ & $664.91$ & $670.58$ \\
\bottomrule
\end{tabular}
\end{table}
两点观察：
\begin{itemize}[nosep]
  \item \textbf{Profile 区间显著非对称}：$|\vec{p}|$ 上 $\hat{p} - q^- = 7.02$ 而 $q^+ - \hat{p} = 0.02$，对应 $\chi^2$ 在 $|\vec{p}|$ 增大方向极陡（$\chi^2_\mathrm{red} = 20.4$ 已经很大，再增 $|\vec{p}|$ 更糟），但向下方向有较深的低 $\chi^2$ 阱。这与 Fisher 的对称 $\pm 2.84\;\mathrm{MeV}/c$ 假设不符——Profile 在此事件上比 Fisher 提供了更真实的不确定性。
  \item Profile $p_x$ 区间几乎完全在 $[-206.5, -200.8]$，即 $\hat{p}_x$ 是区间下限——表示 $p_x$ 朝 $-\infty$ 方向无法进一步减小（边界效应）。
\end{itemize}
...
\begin{algorithm}[H]
\caption{方法四：Laplace 后验协方差}
\label{alg:laplace}
\begin{algorithmic}[1]
\Require LM/MAP 收敛态 $\hat{\vec{\alpha}}$、Jacobian $J$、（可选）动量高斯先验 $(p_0, \sigma_\mathrm{prior})$
\Ensure 后验协方差 $\Sigma_\mathrm{post}$ 与各 $(p_x, p_y, p_z, |\vec{p}|)$ 区间
\State $\Lambda_\mathrm{prior} \gets$ \textbf{if} 先验启用 \textbf{then} $\mathrm{diag}(0, \dots, 1/\sigma_\mathrm{prior}^2)$ \textbf{else} $\mathbf{0}$
\State $\mathcal{H}_\mathrm{post} \gets J^T J + \Lambda_\mathrm{prior}$
\State $\Sigma_\mathrm{post} \gets$ \Call{SVDInverse}{$\mathcal{H}_\mathrm{post}$}（同 Fisher 的 $3{\times}3$ 或 $5{\times}5$ 求逆）
\If{$\Sigma_\mathrm{post}$ 是 PSD}
  \State 走方法二的 256 点参数空间 MC，得到经验 68/95 区间
\Else
  \State 走 Jacobian 线性传播（$\Sigma_{\vec{p}} = M \Sigma_\mathrm{post} M^T$），高斯对称区间
\EndIf
\State \textbf{return} $\Sigma_\mathrm{post}$ 与各分量区间
\end{algorithmic}
\end{algorithm}

代码锚点：\texttt{PDCErrorAnalysis::ComputeBayesianPosterior}（接口）；
$\Sigma_\mathrm{post}$ 求逆与 256 点 MC 共用 \texttt{PDCRkAnalysisInternal.cc} 的内部例程；
先验配置由 \texttt{BayesianConfig::momentum\_prior}（\texttt{MomentumPrior} 结构）控制。

\paragraph{Worked example: event \#399。}
本 run 未启用动量高斯先验（\texttt{MomentumPrior::enabled = false}），因此 $\Lambda_\mathrm{prior} = \mathbf{0}$，$\Sigma_\mathrm{post}$ 数学上等于 $\Sigma_\mathrm{Fisher}$。CSV 输出：

\begin{itemize}[nosep]
  \item Laplace 68\% 区间（\texttt{laplace\_p\_lower68 / upper68}）= $[664.91,\;670.58]$ MeV/$c$，半宽 $\approx 2.84$ MeV/$c$；
  \item Fisher $\sigma_p$ = 2.84 MeV/$c$（完全一致——这正是"无先验时 Laplace 退化为 Fisher"的数值证据）。
\end{itemize}

注意：CSV 中的 \texttt{laplace\_*} 列在状态协方差 PSD 时\textbf{实际承载的是方法二的 256 点 MC 经验分位数}（见 §\ref{sec:mc_prop} 末尾"在本 run 中的实际身份"）。在 event \#399 上，由于动量与 $(u,v,p)$ 之间的非线性微弱（动量较低、轨迹较直），MC 经验分位与 Gaussian-projection 几乎重合，所以"Laplace 区间 $\equiv$ Fisher 半宽"看起来仍然成立。在更高动量、更非线性事件上两者会出现轻度非对称差异。
...
\begin{algorithm}[H]
\caption{方法五：自适应 Metropolis-Hastings（per track）}
\label{alg:mcmc}
\begin{algorithmic}[1]
\Require LM 中心点 $\hat{\vec{\alpha}}$、Laplace $\Sigma_\mathrm{post}$、burn-in $N_b = 5000$、采样 $N_s = 50000$、thin = 5
\Ensure $\sim 10000$ 个后验样本与 Geweke $z$、ESS 诊断
\State $\Sigma_\mathrm{post}^{1/2} \gets$ \Call{SymmetricSqrt}{$\Sigma_\mathrm{post}$}；提议缩放因子 $s \gets 1$
\State $\vec{\alpha}^{(0)} \gets \hat{\vec{\alpha}}$；$\log\pi^{(0)} \gets -\tfrac{1}{2}\chi^2(\vec{\alpha}^{(0)}) + \log \pi_0(\vec{\alpha}^{(0)})$
\For{$t = 1$ \textbf{to} $N_b + N_s$}
  \State $\vec{z} \sim \mathcal{N}(\vec{0}, I)$；$\vec{\alpha}' \gets \vec{\alpha}^{(t-1)} + s \cdot \Sigma_\mathrm{post}^{1/2} \vec{z}$
  \State $\log\pi' \gets -\tfrac{1}{2}\chi^2(\vec{\alpha}') + \log \pi_0(\vec{\alpha}')$（一次完整 RK 传播）
  \State 以概率 $\min(1, e^{\log\pi' - \log\pi^{(t-1)}})$ 接受；否则 $\vec{\alpha}^{(t)} \gets \vec{\alpha}^{(t-1)}$
  \If{$t \leq N_b$ \textbf{且} $t \bmod 50 = 0$}
    \State 根据近 50 步接受率调节 $s$（$> 23\%$ 时 $\times 1.1$；$\leq 23\%$ 时 $\times 0.9$）
  \EndIf
  \If{$t > N_b$ \textbf{且} $(t - N_b) \bmod 5 = 0$}
    \State 储存样本 $\vec{\alpha}^{(t)}$
  \EndIf
\EndFor
\State 计算 \texttt{ComputeGewekeZScore} 与 \texttt{ComputeEffectiveSampleSize}；汇总样本经验分位
\State \textbf{return} 区间 + acceptance + Geweke $z$ + ESS + \texttt{converged} 布尔
\end{algorithmic}
\end{algorithm}

代码锚点：\texttt{PDCErrorAnalysis::ComputeBayesianPosterior}（与 Laplace 共用）；
\texttt{PDCErrorAnalysis::ComputeGewekeZScore}（前 10\% 与后 50\% 均值差的 $z$）；
\texttt{PDCErrorAnalysis::ComputeEffectiveSampleSize}（自相关求和截断在 $|\hat{\rho}_k| < 0.05$）；
收敛判据 \texttt{converged} = (\texttt{ESS} $\geq 50$ AND $|z| < 2$)，输出到 \texttt{bayesian\_samples.csv}。

\paragraph{Worked example: event \#399。}
\texttt{bayesian\_samples.csv} 给出该事件的链诊断：

\begin{table}[H]
\centering
\caption{Event \#399 MCMC 链诊断（quartile=0 抽样事件）}
\label{tab:mcmc_event399}
\begin{tabular}{lc}
\toprule
量 & 值 \\
\midrule
MCMC 68\% 区间 ($|\vec{p}|$, MeV/$c$) & $[643.19,\;654.27]$ \\
区间宽 & $11.08$ \\
MCMC 中心 vs.\ LM 中心 ($|\vec{p}|$, MeV/$c$) & $\sim 648.7$ vs.\ $667.75$（偏离 $\sim 19$） \\
acceptance rate & $0.593$ \\
ESS（有效样本数） & $4.28$ \\
Geweke $|z|$ & $4.17$ \\
\texttt{converged} 标志 & \textbf{0}（未收敛） \\
\bottomrule
\end{tabular}
\end{table}

\textbf{解读}：链未收敛——ESS 仅 4.28（远低于阈值 50），Geweke $|z| = 4.17$（远高于 2.0）。MCMC 后验中心偏离 LM 中心约 19 MeV/$c$、宽度比 Fisher 大约 2$\times$，但这\textbf{不应被解释为"真实后验更宽"}——而是链未充分探索后验，停留在了一个 LM 之外的局部高密度区域。诊断信号已经显式声明本事件 MCMC 不可信。Acceptance 0.593 高于 Metropolis 多维高斯目标的 0.234 最优值，提示 proposal 步长偏小、走步太"近视"。

\textbf{补救方向}：burn-in 加长至 $\geq 20000$；proposal 缩放因子初值放大至 $s = 2$；或改用 NUTS/HMC，让 leapfrog 自适应步长摆脱局部峰。
...
\begin{algorithm}[H]
\caption{方法六：PDC 端到端测量误差传播}
\label{alg:mc_pdc}
\begin{algorithmic}[1]
\Require 中心拟合 $\hat{\vec{\alpha}}$、PDC 测量误差模型 $(\sigma_u, \sigma_v, \rho)$、样本数 $N = 256$
\Ensure $(p_x, p_y, p_z, |\vec{p}|)$ 经验区间 + 采样协方差
\For{$k = 1$ \textbf{to} $256$}
  \State $(\delta u_1, \delta v_1) \sim \mathcal{N}(\vec{0}, \Sigma_\mathrm{wire})$；同样为 PDC2 采样
  \State $\vec{r}_k^{(\mathrm{PDC1,2})} \gets \vec{r}_k^\mathrm{meas} + \delta u \hat{u} + \delta v \hat{v}$（沿丝方向施加扰动）
  \State 以 $\hat{\vec{\alpha}}$ 为初始化，对扰动 PDC 命中跑\textbf{完整 LM 优化}（含 RK4 + Jacobian），得 $\vec{p}^{(k)}$
\EndFor
\State 异常剔除：按 $|\vec{p}|$ 双侧 5\% 截尾，移除 fitter 收敛到错误解的样本
\State 取经验分位 $q_{16/84}$、$q_{2.5/97.5}$，并计算样本协方差矩阵
\State \textbf{return} 区间 + 协方差
\end{algorithmic}
\end{algorithm}

代码锚点：\texttt{PDCErrorAnalysis::RunMonteCarlo}（顶层），扰动子例程 \texttt{PDCErrorAnalysis::PerturbTrack}，每次重建调用同一个 \texttt{PDCMomentumReconstructor}（成员 \texttt{fReconstructor}）；
误差模型由 \texttt{PDCErrorModel}（\texttt{sigma\_u\_mm}、\texttt{sigma\_v\_mm}、\texttt{uv\_correlation}）控制；
样本数与 RNG 由 \texttt{MonteCarloConfig} 控制。

\paragraph{Worked example: event \#399。}
\textbf{此 run 中方法六未被启用}（\texttt{analyze\_pdc\_rk\_error.cc} 默认配置只跑方法一—五）。如果启用，预计每事件 $\sim 25\;\mathrm{s}$（256 次完整 LM），对 815 条 track 大约 $5.7$ 小时；适合作为离线小样本（$\sim 50$ 事件）验证。
对 event \#399，方法六会回答这样的问题：\textbf{若 PDC 丝坐标 $\sigma_u = \sigma_v = 2\;\mathrm{mm}$ 改为 $\sigma_u = \sigma_v = 0.5\;\mathrm{mm}$（更精的探测器），重建动量分布会窄多少？} Fisher 给出的预测是 $\sigma_p \propto \sigma_\mathrm{wire}$（线性约 $4\times$ 改善），但 fitter 在低 $\chi^2$ 多极小值附近的稳健性变化只能由方法六实测——这是它最重要的应用场景。

\subsection{方法总结}

\begin{table}[H]
\centering
\caption{六种误差分析方法对比（方法层面）}
...
\subsubsection*{在同一 run 上的数值对比}

下表把六种方法放到同一份数据上比较：\texttt{pdc\_rk\_error\_after\_pdcana\_20260411}（815 条 \texttt{kThreePointFree} 成功 track，median reco $|\vec{p}| = 1370\;\mathrm{MeV}/c$，$|\vec{p}|$ 四分位 $[971,\;2921]\;\mathrm{MeV}/c$）。$|\vec{p}|$ 的 68\% 区间宽度按 \texttt{summary.csv} 输出的 median 报告：

\begin{table}[H]
\centering
\caption{六种方法在同一 run（\texttt{pdc\_rk\_error\_after\_pdcana\_20260411}）上的 $|\vec{p}|$ 68\% 区间宽度}
\label{tab:error_results}
\begin{tabular}{lccp{6.0cm}}
\toprule
方法 & 样本量 & median 68\% 宽度 (MeV/$c$) & 备注 \\
\midrule
Fisher          & 815（全样本）           & $48.20$  & 每条 track 实时计算 \\
Laplace         & 815（全样本）           & $48.14$  & 每条 track 实时计算；与 Fisher 几乎重合 \\
Profile LH      & 32（按 $|\vec{p}|$ 四分位分层抽样）  & $76.91$  & 离线扫描；明显宽于 Fisher/Laplace \\
MCMC            & 16（同分层抽样）        & $109.32$ & median acceptance $0.59$，median ESS $9.2$；多数链未充分收敛 \\
MC 传播 ($u,v,p$)  & ---                     & ---      & 此 run 未启用；对 $(u,v,p) \to (p_x,p_y,p_z)$ 的非线性进行 256 点采样的代数传播，可加入而几乎不增成本 \\
MC PDC 端到端     & ---                     & ---      & 此 run 未启用；扰动 PDC 测量并完整重建，$\sim 25\;\mathrm{s}$/事件，仅适合离线小样本验证 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{对应输出文件：}\texttt{summary.csv}（聚合 median）、\texttt{track\_summary.csv}（每条 track 的 Fisher 与 Laplace）、\texttt{profile\_samples.csv}（32 条 Profile）、\texttt{bayesian\_samples.csv}（16 条 MCMC + Laplace 95\%）。

\paragraph{Fisher / Profile 全样本宽度分布。}
\begin{figure}[H]
\centering
\begin{minipage}[t]{0.48\textwidth}
\centering
\includegraphics[width=\linewidth]{\AnaRoot/plots/fisher_p_width68.png}
\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}
\centering
\includegraphics[width=\linewidth]{\AnaRoot/plots/profile_p_width68.png}
\end{minipage}
\caption{Fisher 68\% 半宽 vs reco $|\vec{p}|$（左图，815 条 track）与 Profile 68\% 半宽（右图，32 条分层抽样）。横轴 reco $|\vec{p}|$ 由默认 \texttt{kThreePointFree} 模式（5 参 $\delta x, \delta y, u, v, p$）的 LM/RK4 拟合给出，所有 815 条 track 共享同一中心估计；4 种方法（Fisher/Laplace/Profile/MCMC）只是给该中心估计加上不同形式的 68\%/95\% 区间。}
\label{fig:fisher_profile}
\end{figure}

\paragraph{Laplace 与 MCMC 对比。}
\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{\AnaRoot/plots/laplace_vs_mcmc_p_width68.png}
\caption{Laplace 68\% 半宽 vs MCMC 68\% 半宽（同一批 16 条分层抽样事件）。中心动量同样来自 \texttt{kThreePointFree} 拟合；CSV 中的 \texttt{laplace\_*} 列在状态协方差 PSD 时实际承载方法二的 256 点参数空间 MC 经验分位（见 §\ref{sec:mc_prop} 末尾）；MCMC 在大多数事件上系统性更宽，反映非高斯尾部 + 链未充分收敛的混合效应。}
\label{fig:laplace_vs_mcmc}
\end{figure}

\paragraph{结果讨论。}
\begin{itemize}[nosep]
  \item \textbf{Fisher $\approx$ Laplace}（48.20 vs 48.14 MeV/$c$）：因为本 run 未启用 $|\vec{p}|$ 高斯先验（\texttt{MomentumPrior::enabled = false}），Laplace 矩阵 $(J^TJ + \Lambda_\mathrm{prior})^{-1}$ 退化为纯 $(J^TJ)^{-1}$，与 Fisher 数学等价。两者间的 $0.06\;\mathrm{MeV}/c$ 差异源于数值实现细节（白化方式、SVD 截断阈值），可作为实现一致性的 sanity check。
  \item \textbf{Profile $>$ Fisher}（76.9 vs 48.2 MeV/$c$，宽 60\%）：Profile 不依赖局部高斯近似。Fisher 的窄宽提示 $\chi^2$ 在 $|\vec{p}|$ 方向的真实形状\textbf{并非抛物线}——存在显著非对称尾部（与 §\ref{sec:profile} 的 Wilks 定理推导所要求的"$\chi^2$ 在最优点附近平方衰减"假设矛盾）。代表性 profile 曲线见 \texttt{representative\_profile\_curves.png}（已在 \texttt{pdc\_reconstruction\_uncertainty\_detailed.pdf} 第 11 帧展示）。
  \item \textbf{MCMC $>$ Profile}（109.3 vs 76.9 MeV/$c$）：median acceptance 0.59 略高于 Metropolis 的最优值 0.234（提示步长偏小、链探索不充分），median ESS 仅 9.2（远小于 50000 步原始样本应达到的有效数）。多数链 \texttt{converged=0}（Geweke $|z| > 2$）。MCMC 区间在此 run 中\textbf{不可信}，需调长 burn-in、加大 proposal scale，或使用 NUTS / HMC 替代。
  \item \textbf{覆盖率验证缺位}：本 run 没有 truth 信息（见 §\ref{sec:error} 范围说明）。Fisher、Laplace、Profile、MCMC 在 truth-grid 上的实际覆盖率请参考 2026-04-17 的 ensemble study——结论为四种方法在 68\% 处全部欠覆盖，主因是 $dE/dx$ 中心值偏置 + $(u,v,p) \to (p_x, p_y, p_z)$ 局部线性化误差。
\end{itemize}
...
\begin{tabular}{lccc}
\toprule
指标 & 理论（$\sigma_x = 0.5$~mm） & NN $p_z$ MAE \textcolor{red}{(污染)} & RK $p_z$ MAE \\
\midrule
...
\noindent\textcolor{red}{\textbf{审计注（2026-04-20）：} 下表 NN 偏差列使用的是
\emph{已删除的污染模型} \texttt{formal\_B115T3deg\_qmdwindow/20260227\_223007/}
（见 \texttt{docs/audit/2026-04-20-nn\_target\_contamination\_audit.md}）。清洁重训
后验证集 $p_x$ RMSE $-51\%$、$p_y$ $-81\%$、$p_z$ 基本不变。因此 $p_z$ 比较
（RK $-10.8$ vs NN $-3$，$dE/dx$ 主导）结论仍然成立，但"$p_x$/$p_y$ 两种方法一致"
的结论无效 —— 清洁 NN 在横向动量上会明显优于 RK。重新引用时请用
\texttt{formal\_B115T3deg/20260420\_184345/model/model.pt} 重跑
\texttt{test\_rk\_nn\_comparison.py}。}

\begin{table}[H]
\centering
\caption{E2E 重建偏差对比（单位：MeV/$c$；\textcolor{red}{NN 列基于污染模型}）}
\label{tab:e2e}
...
\paragraph{关键结论。}
\begin{itemize}[nosep]
  \item $p_x$ / $p_y$ 偏差在本表中两种方法几乎一致，\textcolor{red}{但 NN 侧使用的是污染模型；清洁模型下 NN 在横向动量上会明显更优}。
  \item $p_z$ 方面 RK 偏差（$5\text{--}16\;\mathrm{MeV}/c$）大于 NN（$0.2\text{--}6.3\;\mathrm{MeV}/c$），且呈系统性负偏差。原因是 RK 轨迹模型中 $|\vec{p}|$ 守恒，未考虑质子在探测器气体中的 $dE/dx$ 能量损失（$\sim 50\;\mathrm{MeV}$ 系统偏移）。\textcolor{red}{$p_z$ 在污染前后近乎不变，本结论仍成立。}
...
\begin{enumerate}
  ...
  \item \textbf{当前精度}（来自 E2E 测试 CSV）：RK MAE = $(20.4,\;16.0,\;10.8)\;\mathrm{MeV}/c$（$p_x, p_y, p_z$），NN MAE = $(21.0,\;15.6,\;3.1)\;\mathrm{MeV}/c$ \textcolor{red}{[NN 来自污染模型，见 2026-04-20 审计；清洁 NN 在 $p_x,p_y$ 上显著更优，$p_z$ 几乎不变]}。Fisher $\sigma_{|\vec{p}|}$ = 2.4--6.0 MeV/$c$。$p_z$ 残余系统偏差（均值 $-10.8\;\mathrm{MeV}/c$）源于未建模的 $dE/dx$ 能量损失。
