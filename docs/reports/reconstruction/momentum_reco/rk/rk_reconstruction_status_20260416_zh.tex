...
\end{minipage}
\caption{Fisher 68\% 半宽 vs reco $|\vec{p}|$（左图，815 条 track）与 Profile 68\% 半宽（右图，32 条分层抽样）。}
...
\caption{Laplace 68\% 半宽 vs MCMC 68\% 半宽（同一批 16 条分层抽样事件）。MCMC 在大多数事件上系统性更宽，反映非高斯尾部 + 链未充分收敛的混合效应。}
