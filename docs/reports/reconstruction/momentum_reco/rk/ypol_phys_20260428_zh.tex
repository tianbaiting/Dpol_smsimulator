...
\end{itemize}
\section{4. Per-(target, gamma) sample 统计}
...
\midrule
Sn112E190 & g050 & 20000 & 10309 & 8850 & 821 & 147 \\
Sn112E190 & g060 & 20000 & 6172 & 5244 & 575 & 88 \\
Sn112E190 & g070 & 20000 & 4980 & 4168 & 525 & 97 \\
Sn112E190 & g080 & 20000 & 3763 & 3153 & 409 & 76 \\
Sn124E190 & g050 & 20000 & 11307 & 9663 & 832 & 210 \\
Sn124E190 & g060 & 20000 & 6795 & 5757 & 591 & 145 \\
Sn124E190 & g070 & 20000 & 4740 & 4023 & 498 & 111 \\
Sn124E190 & g080 & 20000 & 3916 & 3282 & 427 & 109 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{读法}: tight 通过率 ~2\textendash6\%; NEBULA 在 tight 子集上击中率 ~20\%。
\section{5. 主结果: $R_x^\text{rot}$ vs gamma （反应平面 frame, ynp+ypn pooled）}
...
\midrule
Sn112E190 & g050 & 0.232 & 0.245 & 0.835 & 0.257 & 0.272 & 0.815 & 0.301 & 0.324 & 1.492 \\
Sn112E190 & g060 & 0.218 & 0.226 & 0.721 & 0.241 & 0.250 & 0.722 & 0.250 & 0.264 & 1.514 \\
Sn112E190 & g070 & 0.211 & 0.222 & 0.787 & 0.232 & 0.245 & 0.757 & 0.199 & 0.221 & 0.764 \\
Sn112E190 & g080 & 0.218 & 0.227 & 0.735 & 0.242 & 0.252 & 0.725 & 0.196 & 0.203 & 0.652 \\
Sn124E190 & g050 & 0.285 & 0.296 & 0.887 & 0.321 & 0.334 & 0.885 & 0.619 & 0.628 & 1.658 \\
Sn124E190 & g060 & 0.265 & 0.277 & 0.945 & 0.297 & 0.309 & 0.912 & 0.431 & 0.459 & 1.266 \\
Sn124E190 & g070 & 0.255 & 0.264 & 0.789 & 0.284 & 0.295 & 0.786 & 0.431 & 0.435 & 1.018 \\
Sn124E190 & g080 & 0.223 & 0.232 & 0.882 & 0.250 & 0.258 & 0.857 & 0.318 & 0.339 & 0.879 \\
\bottomrule
\end{tabular}
\end{table}
...
\end{itemize}
\subsection*{5.2 R vs gamma 图（每 cut 一张）}
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/ypol_phys/r_vs_gamma_loose.png}
\caption{$R_x^\text{rot}$ 随 gamma (loose cut, ynp+ypn pooled)。两 panel = 两 target；每 panel 三条曲线: 黑圆实线 = A truth, 橙方虚线 = B reco mixed, 蓝三角点线 = C full reco。\textbf{所有 R 值在反应平面 frame}。}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/ypol_phys/r_vs_gamma_mid.png}
\caption{$R_x^\text{rot}$ 随 gamma (mid cut, ynp+ypn pooled)。两 panel = 两 target；每 panel 三条曲线: 黑圆实线 = A truth, 橙方虚线 = B reco mixed, 蓝三角点线 = C full reco。\textbf{所有 R 值在反应平面 frame}。}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/ypol_phys/r_vs_gamma_tight.png}
\caption{$R_x^\text{rot}$ 随 gamma (tight cut, ynp+ypn pooled)。两 panel = 两 target；每 panel 三条曲线: 黑圆实线 = A truth, 橙方虚线 = B reco mixed, 蓝三角点线 = C full reco。\textbf{所有 R 值在反应平面 frame}。}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{figures/ypol_phys/px_diff_hist_Sn112E190_loose.png}
\caption{$\Delta p_x^\text{rot}$ 分布 (Sn112E190, loose cut, ynp+ypn pooled)。黑 step = A truth, 橙填充 = B reco mixed, 蓝虚线 = C full reco.}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{figures/ypol_phys/px_diff_hist_Sn124E190_loose.png}
\caption{$\Delta p_x^\text{rot}$ 分布 (Sn124E190, loose cut, ynp+ypn pooled)。黑 step = A truth, 橙填充 = B reco mixed, 蓝虚线 = C full reco.}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{figures/ypol_phys/px_diff_hist_Sn112E190_mid.png}
\caption{$\Delta p_x^\text{rot}$ 分布 (Sn112E190, mid cut, ynp+ypn pooled)。黑 step = A truth, 橙填充 = B reco mixed, 蓝虚线 = C full reco.}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{figures/ypol_phys/px_diff_hist_Sn124E190_mid.png}
\caption{$\Delta p_x^\text{rot}$ 分布 (Sn124E190, mid cut, ynp+ypn pooled)。黑 step = A truth, 橙填充 = B reco mixed, 蓝虚线 = C full reco.}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{figures/ypol_phys/px_diff_hist_Sn112E190_tight.png}
\caption{$\Delta p_x^\text{rot}$ 分布 (Sn112E190, tight cut, ynp+ypn pooled)。黑 step = A truth, 橙填充 = B reco mixed, 蓝虚线 = C full reco.}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{figures/ypol_phys/px_diff_hist_Sn124E190_tight.png}
\caption{$\Delta p_x^\text{rot}$ 分布 (Sn124E190, tight cut, ynp+ypn pooled)。黑 step = A truth, 橙填充 = B reco mixed, 蓝虚线 = C full reco.}
\end{figure}
\subsection*{6.2 Cuts overlay（一图叠加 4 cuts: 无 cut + loose + mid + tight, ynp+ypn pooled）}
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/ypol_phys/px_diff_cuts_overlay_Sn112E190.png}
\caption{$\Delta p_x^\text{rot}$ 在四 cut 下的叠加 (Sn112E190, ynp+ypn pooled)。4 行=4 gammas, 列=truth (A) / reco mixed (B). 蓝=无 cut, 绿=loose, 橙=mid, 红=tight.}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/ypol_phys/px_diff_cuts_overlay_Sn124E190.png}
\caption{$\Delta p_x^\text{rot}$ 在四 cut 下的叠加 (Sn124E190, ynp+ypn pooled)。4 行=4 gammas, 列=truth (A) / reco mixed (B). 蓝=无 cut, 绿=loose, 橙=mid, 红=tight.}
\end{figure}
...
\begin{tabular}{lr}
\toprule
mean R\_truth\_tight (8 cells, ynp+ypn pooled)            & 3.365 \\
mean R\_full\_uncorr (NEBULA-only)         & 0.954 \\
mean R\_full\_corr ($1/\varepsilon$ weighted)  & 1.083 \\
\midrule
truth − corr 缺口                          & +2.282 \\
truth − uncorr 缺口                        & +2.411 \\
\bottomrule
\end{tabular}
\end{table}
