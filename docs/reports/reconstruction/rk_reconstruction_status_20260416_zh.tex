...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{samurai_geometry_xz.png}
...
\begin{itemize}[nosep]
  ...
  \item \texttt{.../profile\_samples.csv}：32 条 Profile LH（px, py, pz, $|p|$ 全分量）；
  \item \texttt{.../bayesian\_samples.csv}：64 条 MCMC（当前分析器仅对 $|p|$ 采样，见下）；
...
\caption{四种方法算出的误差与真实残差对照（MeV/$c$；偏差=residual 中位数；残差半宽=鲁棒 $(p_{84}-p_{16})/2$）}
...
\textbf{如何读}（关掉 Sn 靶之后的新画像）：
\begin{itemize}[nosep]
  \item 四种方法给出的半宽度相互\emph{相差不到 $20\%$}（Fisher/Laplace 最大，Profile 次之，MCMC 最窄）。
  \item $|\vec p|$ 和 $p_z$：半宽度与真实残差半宽\emph{基本一致}（比值 $0.83$--$1.26$），观测 68\% 覆盖率落在 CP 95\% CI 里，名义 0.68 也在 CI 里——这两个方向 Fisher 真的管用。
  \item $p_y$：半宽度 $0.77$ vs.\ 残差半宽 $1.40$，Fisher 偏\emph{乐观}约 $1.8\times \Rightarrow$ 覆盖率 43\%。根因是 $(u,v,p)$ 参数化在 $p_y$ 方向的 Fisher 信息被压低。
  \item $p_x$：\emph{不是半宽度问题}，残差半宽 $6.21$ 与 Fisher $7.52$ 只差 $20\%$；真正的毛病是 $p_x$ \emph{中位偏差 $-33\;\mathrm{MeV}/c$}——truth 永远落在 reco 下方 $\sim\!4.4\sigma$ 处，覆盖率塌到 5\%。这是重建本身的系统误差，不是误差棒大小的问题，任何给误差棒加宽都治不好。
\end{itemize}
...
\textbf{如何读表（以第一行为例）}：对 744 条事件，Fisher 给出的 68\% $|\vec p|$ 区间里有 $744\times 0.761=566$ 条包含了真值。观测覆盖率 0.761 的 Clopper-Pearson 95\% CI 是 $[0.728, 0.791]$——名义 0.68 刚好落在 CI 下方外侧，说明 Fisher $|\vec p|$ 是略偏保守的（而不是像老版本那样严重欠覆盖）。$p_x$ 那行 CP CI $[0.038, 0.071]$ 则把名义 0.68 甩得很远——覆盖率显著低于名义值在统计上是稳的，不是样本量不足造成的涨落。
...
\textbf{关键发现}（关掉 Sn 靶之后画像明显分成三挡）：
\begin{itemize}[nosep]
  \item \textbf{$|\vec p|, p_z$ 达标}：Fisher $|\vec p|$ 68\% 覆盖率 0.76、$p_z$ 0.68——曲率约束在关掉靶物质多次散射后就足以把误差棒匹配到真实残差分布，名义 0.68 都在 CP CI 之内或紧邻。$|\vec p|$ 略偏保守。
  \item \textbf{$p_y$ 轻度欠覆盖}：0.43 相对名义 0.68 差 $\sim\!2\times$——残差半宽 $1.40$ vs.\ Fisher $0.77$，说明 $(u,v,p)$ 参数化在磁场主弯平面外的 Fisher 信息被压低了约 2 倍。
  \item \textbf{$p_x$ 几乎不覆盖}：0.052（Profile 甚至 0/32）——这不是误差棒太小，是 reco 中位数比 truth 系统性地低 $33\;\mathrm{MeV}/c$，每个事件都偏差 $\sim 4.4\sigma$。这是重建系统误差，扩大误差棒不可能修得了。
\end{itemize}

\textbf{根因分析}：
\begin{itemize}[nosep]
  \item $p_x$ 系统偏差：主要是 RK 传播缺少 $dE/dx$ 能量损失建模、加上 $(u,v,p)$ 非线性 Jacobian 把 $p_x$ 方向最佳拟合推到一个系统偏低的位置。需要加 Bethe-Bloch + 换回 Cartesian $(p_x,p_y,p_z)$ 参数化直接拟合。
  \item $p_y$ Fisher 低估 $1.8\times$：同一套 $(u,v,p)$ 线性化问题，Cartesian 重拟合应该能一次性修掉。
  \item MCMC/Profile 的覆盖率和 Fisher 一致（Profile $p_x$ 甚至 0/32）确认了\emph{问题在共享的前向模型，不是某一种区间方法}的缺陷。
\end{itemize}

这组去掉 Sn 靶后的 744 事件覆盖率把报告从"数据编造"纠偏到"可重复、可复核"：$|\vec p|/p_z$ 已经够用，$p_x/p_y$ 的剩余问题指向了清晰的前向模型改造路径（$dE/dx$ + Cartesian refit），先纠根因再扩到 89k 级数据集。

\paragraph{MCMC 诊断。}
本次 ensemble（\texttt{summary.csv}）：MCMC 中位接受率 0.32，中位有效样本量 ESS 9.02。ESS 如此低说明 160 样本/80 burn-in 远远不够；建议增至 $\sim\!10^4$ 样本再复核 MCMC 覆盖率。
...
\paragraph{数据处理。}
从 reco ROOT 文件 \texttt{pdc\_truth\_grid\_rk\_fixed\_target\_pdc\_only\_reco.root} 的 \texttt{RecoEvent.tracks[].start / .end} 提取 PDC1 / PDC2 全局坐标，按 \texttt{event\_index} 与 \texttt{track\_summary.csv} 内连接。脚本：
\begin{itemize}[nosep]
  \item \texttt{scripts/analysis/extract\_pdc\_hits\_from\_reco.py}（PyROOT $\to$ CSV）
  \item \texttt{scripts/analysis/plot\_pdc\_hit\_dispersion.py}（5 个特殊点各一张图）
\end{itemize}
合表：\texttt{build/test\_output/ensemble\_n50/g4\_fluctuation/ensemble\_pdc\_reco\_merged.csv}（744 行，缺失 0）。
...
\paragraph{传递关系。}
RK4 fitter 做的是 $\vec p_\mathrm{target} \mapsto (\vec r_\mathrm{PDC1}, \vec r_\mathrm{PDC2})$ 的逆向拟合。\textbf{Fisher 矩阵里假设的输入 $\sigma$ 只有 $\sigma_\mathrm{wire}\!\sim\!0.2$ mm}（丝坐标分辨率），据此算出的 Fisher $\sigma_p$ 是几 MeV/$c$。关掉 Sn 靶后 PDC 平面弥散降到 $3$--$15$ mm——仍比 $\sigma_\mathrm{wire}$ 大 $15$--$75\times$，但不再是带靶时 $\sim\!500\times$ 的灾难。全局 G4/Fisher 半宽比变成 $p_x{:}0.83$, $p_y{:}1.82$, $p_z{:}1.03$, $|\vec p|{:}0.99$——三个分量已经基本"配对"，只有 $p_y$ 还有 1.8× 系统性欠估。$p_x$ 方向的残差大头反而是\emph{中位偏差}（$-33\;\mathrm{MeV}/c$），不是误差棒宽度。
...
\caption{基线 $(0,0,627)$ 真值下 49 份 G4 重复事件的 reco $(p_x, p_y, p_z)$ 分布。$p_x$ 中位数在 $\sim\!-32\;\mathrm{MeV}/c$（偏离 truth），G4 $\sigma=4.7$ vs.\ Fisher $\sigma=7.2$，比值 0.66——\emph{误差棒反而偏大}但中位数偏得离谱；$p_y$ G4=1.5 vs.\ Fisher=0.8，比值 1.9（唯一 Fisher 欠估的方向）；$p_z$ G4=4.1 vs.\ Fisher=5.8，比值 0.71（基本吻合）。少量外点基本不影响鲁棒半宽度。}
...
\caption{$(+100, 0, 627)$ 真值下 50 份 G4 重复事件。$p_x$ G4=4.0 vs.\ Fisher=6.9，比值 0.58（Fisher 相对保守）；$p_y$ G4=1.4 vs.\ Fisher=0.5，比值 2.8（这是 15 个真值点中 Fisher $\sigma_{p_y}$ 最小、实际 G4 典型的地方，所以比值最大）；$p_z$ G4=5.1 vs.\ Fisher=7.3，比值 0.70。不同真值点的 Fisher/残差比值确实有 $\sim 2\text{-}3\times$ 的点到点差异——下表汇总。}
...
\caption{完整 $5\times 3$ 真值网格的 reco $p_x$ 直方图（行 $p_y \in \{+20,0,-20\}$，列 $p_x \in \{-100,-50,0,50,100\}\;\mathrm{MeV}/c$）。每格黑线 = 真值、红虚线 = G4 中位数、粉带 = Fisher $\sigma$；标注 $r$ = G4 $\sigma$ / Fisher $\sigma$。中位偏差典型值 $-20$ 到 $-40\;\mathrm{MeV}/c$；$(-100,0)$ 格因 LM 掉进粗格盆地而反向跳出，是整张网格的异常点。宽度本身与 Fisher 一致（比值 $\sim 1$）——\textbf{$p_x$ 的病主要是位置偏差而非误差棒。}}
...
\caption{同样布局，分量改为 $p_y$。G4 半宽大致在 $1$--$2\;\mathrm{MeV}/c$，Fisher $\sigma_{p_y}$ 在 $0.5$--$1.0\;\mathrm{MeV}/c$，比值 $1.5$--$2.8$——这是 \textbf{Fisher $\sigma_{p_y}$ 系统性欠估 $\sim 2\times$} 的可视化证据。物理上归因于 $(u,v,p)$ 参数化把 $p_y$ 响应线性化后低估了实际传递涨落。}
...
\caption{同样布局，分量改为 $p_z$。整个网格 G4/Fisher 比值稳定在 $0.7$--$0.8$——Fisher 对 $p_z$ \emph{略微保守}，与全局残差分析得到的 $1.03$ 一致。唯一例外仍是 $(-100,0)$ 格：LM 拟合在 PDC 接收度边缘失败导致 G4 $\sigma\sim 21\;\mathrm{MeV}/c$。所有格的真值 $p_z=627\;\mathrm{MeV}/c$。}
...
\paragraph{横向读表。}
\begin{itemize}[nosep]
  \item 关掉 Sn 靶后 PDC $\sigma_y$ 从 $\sim$100 mm 塌到 $\sim$10--15 mm，对应 reco 动量 G4/Fisher 比值也从老版本的 $3$--$28\times$ 塌到 $p_x,p_z$ 的 $\sim 1$ 附近和 $p_y$ 的 $\sim 2$——\emph{Fisher 宽度定标已基本正确}，唯一剩余的宽度问题在 $p_y$。
  \item $(-100, 0)$ 是单独失控的一个点：$p_x / p_z$ G4 半宽分别飙到 $\sim 36$ 和 $\sim 21\;\mathrm{MeV}/c$（比值 $3.6$）——这就是 LM 粗格 + PDC 接收度边缘"向内偏转"轨迹的已知覆盖率盲区（见项目 memo 中 RK 覆盖率报告）。
  \item 剩下的 $p_x$ 5\%、$p_y$ 43\% 欠覆盖已经被拆分得很清楚：(a) $p_x$ 中位偏差 $\sim\!-30\;\mathrm{MeV}/c$ 拉着 truth 永远出框——这是前向模型"居中位置"的毛病，宽度调整救不了；(b) $p_y$ 的 Fisher $\sigma$ 压低 $\sim 2\times$ 来自 $(u,v,p)$ 线性化。两个问题共同的修复方向是 $dE/dx$ + Cartesian $(p_x,p_y,p_z)$ 直接拟合。
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{修复欠覆盖}（最紧迫）。744 事件（关掉 Sn 靶）ensemble 显示 Fisher 68\% 覆盖率 $|\vec p|\,76\%$、$p_z\,68\%$（已达标）、$p_y\,43\%$（Fisher 偏紧 $1.8\times$）、$p_x\,5\%$（中位偏差 $-33\;\mathrm{MeV}/c$，是前向模型的系统误差）。在扩大数据集之前必须：
  \begin{itemize}[nosep]
    \item 消除 $p_z$ 系统偏差（见上条 $dE/dx$）；
    \item 扩充协方差模型：把 $p_x,p_y$ 非高斯后验纳入（Laplace 近似在 $(u,v,p)$ 参数化下低估曲率 $\sim\!10\times$）；或者在 $(p_x,p_y,p_z)$ 参数化下做直接的 Cartesian 拟合（当前版本未实现，见已知局限 \#5）。
  \end{itemize}
  修复后再在 $89\mathrm{k}$ 事件 NN 训练集上做大统计验证，目标 $[65\%, 70\%]$。
