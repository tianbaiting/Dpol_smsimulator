...
\begin{frame}{Final Accuracy (All Files, All Events) \textcolor{red}{[INVALIDATED]}}
\small
\textcolor{red}{These \texttt{evaluate\_reconstruct\_sn\_nn} RMSEs were produced
by the now-deleted polluted model running over the clean
\texttt{eval\_20260227\_235751/} reconstruction files. Matching efficiency
(geometry acceptance) is unaffected; the RMSE rows must be re-derived with
the 2026-04-20 clean model before quoting.}
\begin{table}
\centering
\begin{tabular}{lccc}
\toprule
...
\textbf{Current performance interpretation} \textcolor{red}{[pending re-eval]}
\begin{itemize}
\item RMSE-based Y-pol / Z-pol comparisons depended on the polluted model
and must be re-derived with the 2026-04-20 clean model.
\item Validation-set comparison (from the clean retrain): \(p_x\) RMSE
\(13.60\rightarrow 6.70\), \(p_y\) RMSE \(12.44\rightarrow 2.40\); full-event
re-eval is expected to follow the same direction.
\item Geometry chain and matching efficiency numbers remain production-grade.
\end{itemize}
\end{frame}

\begin{frame}{Conclusion}
\begin{itemize}
\item Narrowing to the qmd-window sampling significantly improves
effective kept samples (\(\sim24\%\rightarrow\sim45\%\)) --- this is a
geometry-acceptance result and survives the contamination audit.
\item The clean 2026-04-20 retrain on the same narrow window confirms
the window choice: val \(|\vec p|\) RMSE \(= 6.08\) MeV/c with strong
...
\item Follow-up: re-run \texttt{evaluate\_reconstruct\_sn\_nn} over
\texttt{eval\_20260227\_235751/} with the clean model; re-do the
hyperparameter sweep on the clean dataset before claiming a best candidate.
\end{itemize}
\end{frame}
