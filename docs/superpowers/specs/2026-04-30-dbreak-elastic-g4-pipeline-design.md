...
一个 Python 驱动脚本 `scripts/batch/run_dbreak_elastic_pipeline.py`,在本地启动,通过 `ssh spana03` 调远端二进制。五个子命令 + `all` + `status`:

```
prebuild   SSH 远端 git pull origin main + bash build.sh, 验证 libsmdata.so 含 EventTruthConverter
...
CONFIG = {
    ...
    "remote_build_cmd":  "bash build.sh",   # 或者 cmake --build build -j8
...
| GenInputRoot 行为 | zpol: 每事件 random Δ ∈ [0,2π) 旋转 (px,py),写 `b_phi=Δ`;ypol: 不旋转,写 `b_phi=rpphi*π/180` (从 raw QMD 第 9 列) | — | — |
...
python3 scripts/batch/run_dbreak_elastic_pipeline.py \
    {prebuild|sync|prepare|gen-input|gen-output|all|status} \
...
cd /home/tbt/workspace/Dpol_smsimulator && \
  ...
  source setup_spana.sh && bash build.sh 2>&1 | tail -50
```
完成后再 SSH 一次校验:`nm -D build/lib/libsmdata.so | grep EventTruthConverter` 必须非空,否则报错并打印 build 末尾 50 行。写 `state_dir/prebuild.done` JSON `{git_head, build_timestamp, has_event_truth}`。`--force` 时先 `git reset --hard origin/main` 再 build (慎用)。
...
### `all`
依次跑 `prebuild → sync → prepare → gen-input → gen-output`,任一失败终止并打印断点。
...
| 远端 git 有未提交改动 / 不在 main | prebuild 拒绝跑,提示用户先理顺;`--force` 时 `git reset --hard origin/main` |
...
2. `sync --dry-run` 看 rsync 文件列表,确认只匹配 dbreak.dat、目标路径正确
3. `prepare --dry-run` 看打印的 16 条 symlink 计划
...
8. `status` 子命令确认 5 阶段都标 done、g4output 文件数 = 176
