...
**Goal:** 把 158 行的 `README.md` 重写成"首屏路由 + 分层深度文档"的双文件结构，让新贡献者十分钟内看懂 Geant4 模拟 / ROOT 重建 / 几何筛选三条数据流主干。

**Architecture:** 两份文件分工：
- `README.md`（~100 行）= 全局数据流图 + 子系统速览表 + 快速上手 + 指向深度文档的链接
- `docs/SUBSYSTEMS.md`（~600 行，新建）= 每个子系统一节（§1 Geant4 / §2 ROOT reco / §3 几何筛选 / §4 分析+NN / §A 附录），每节含 做什么 / 组件地图 / I-O / 入口命令 / 典型改动点 / 常见坑

**Tech Stack:** Pure Markdown。验证手段：`ls` 检查组件地图里的路径存在，`bin/<exe> --help` 检查示例命令的 ParseArgs 不报错，GitHub 锚点渲染用 `# 1 geant4-模拟` 规则。

**Reference:** 设计 spec 见 `docs/superpowers/specs/2026-04-17-readme-rewrite-design.md`。
...
| 文件 | 动作 | 行数目标 | 责任 |
| --- | --- | --- | --- |
| `README.md` | 重写 | ~100 | 首屏路由；数据流图；子系统速览；快速上手 |
| `docs/SUBSYSTEMS.md` | 新建 | ~600 | §0/§1/§2/§3/§4/§A 子系统深度 |
| `docs/superpowers/specs/2026-04-17-readme-rewrite-design.md` | 已存在 | — | 设计 spec（此次不动） |

SUBSYSTEMS.md 按子系统切分而非按层切分，因为同一子系统的组件地图、I/O、入口命令、坑必须一起读；跨子系统的共性（数据流、坐标系）放 §0。

---

## Task 1: 验证设计 spec 里引用的所有路径存在

**Files:**
- Read: `docs/superpowers/specs/2026-04-17-readme-rewrite-design.md`

**动机：** spec 里组件地图引用了约 50 个文件路径；在动笔前全部验证一遍，避免把错的路径写进最终文档。

- [ ] **Step 1: 提取 spec 中所有 backtick 包围的路径，逐一 `ls` 检查**
...
for p in \
    ...
    libs/sim_deuteron_core \
    libs/smg4lib/include/DipoleConstruction.hh \
    libs/smg4lib/include/PDCConstruction.hh \
    libs/smg4lib/include/NEBULAConstruction.hh \
    libs/smg4lib/src/data/src/FragSimDataInitializer.cc \
    libs/smg4lib/src/data/src/BeamSimDataInitializer.cc \
    libs/smg4lib/include/MagField.hh \
    ...
    libs/analysis_pdc_reco/src/PDCRkAnalysisInternal.cc \
    libs/analysis_pdc_reco/src/PDCErrorAnalysis.cc \
    libs/analysis_pdc_reco/src/PDCNNMomentumReconstructor.cc \
    libs/analysis_pdc_reco/src/PDCSimAna.cc \
    libs/analysis_pdc_reco/include/PDCRecoTypes.hh \
    libs/analysis/include/GeometryManager.hh \
    libs/analysis/include/MagneticField.hh \
    libs/analysis/include/ParticleTrajectory.hh \
    libs/analysis/include/EventDataReader.hh \
    ...
    libs/geo_accepentce/include/BeamDeflectionCalculator.hh \
    libs/geo_accepentce/include/DetectorAcceptanceCalculator.hh \
    libs/geo_accepentce/include/GeoAcceptanceManager.hh \
    scripts/analysis/analyze_ensemble_coverage.py \
    scripts/analysis/compute_per_truth_g4_vs_fisher.py \
    scripts/analysis/extract_pdc_hits_from_reco.py \
    scripts/analysis/plot_pdc_hit_dispersion.py \
    scripts/analysis/plot_reco_momentum_dispersion.py \
    ...
    scripts/batch/batch_run_ypol.py \
    configs/simulation/macros/simulation.mac \
    configs/simulation/geometry/3deg_1.15T.mac \
    configs/simulation/geometry/SAMURAIPDC.xml \
    configs/simulation/geometry/NEBULA_Dayone.csv \
; do
  if [ -e "$p" ]; then echo "OK   $p"; else echo "MISS $p"; fi
done
```

Expected: 每行 `OK ...`。任何 `MISS` 都要在写文档前解决（要么修正路径，要么删除该引用）。

- [ ] **Step 2: 记录任何需要订正的路径**

将 `MISS` 的路径与正确路径记录到本地笔记（不提交），后续 Task 里直接用正确版本。

- [ ] **Step 3: Commit（可选）**

本任务纯验证，无改动，跳过 commit。

---

## Task 2: 创建 docs/SUBSYSTEMS.md 骨架 + §0 全局数据流
...
- [ ] **Step 2: 验证 Markdown 渲染 OK**

Run: `wc -l docs/SUBSYSTEMS.md`
Expected: 约 45–55 行
...
git commit -m "add SUBSYSTEMS.md skeleton with global data flow"
```

---

## Task 3: 写 §1 Geant4 模拟

**Files:**
- Modify: `docs/SUBSYSTEMS.md`（append §1）

- [ ] **Step 1: Append §1 到 SUBSYSTEMS.md**
...
Run: `awk '/^## §1 /,/^## §2 /' docs/SUBSYSTEMS.md | wc -l`
Expected: 约 90–130 行
...
```bash
git add docs/SUBSYSTEMS.md
git commit -m "add SUBSYSTEMS.md §1 Geant4 simulation section"
```
...
| `libs/analysis_pdc_reco/src/PDCSimAna.cc` | sim.root → `PDCInputTrack`（u/v 丝坐标 + z 平面） |
...
```bash
git add docs/SUBSYSTEMS.md
git commit -m "add SUBSYSTEMS.md §2 ROOT reconstruction section"
```
...
```bash
git add docs/SUBSYSTEMS.md
git commit -m "add SUBSYSTEMS.md §3 geometry filter section"
```
...
```bash
git add docs/SUBSYSTEMS.md
git commit -m "add SUBSYSTEMS.md §4 analysis and NN backend section"
```
...
```bash
git add docs/SUBSYSTEMS.md
git commit -m "add SUBSYSTEMS.md §A appendix"
```
...
```bash
git add README.md
git commit -m "rewrite README as data-flow overview + routing to SUBSYSTEMS.md"
```
...
- [ ] **Step 1: 检查 README 到 SUBSYSTEMS.md 的锚点**

Run:
```bash
grep -oE 'docs/SUBSYSTEMS.md#[a-z0-9-]+' README.md | sort -u
```
Expected: 列出所有锚点。肉眼比对 SUBSYSTEMS.md 里的 `## §N ...` 标题，GitHub 的自动锚点规则是去掉 `§`、转小写、空格换 `-`。例如 `## §1 Geant4 模拟` → `#1-geant4-模拟`。

- [ ] **Step 2: 检查所有组件地图中的路径真实存在**

Run:
```bash
# 从 SUBSYSTEMS.md 里抽出所有 backtick 包围的相对路径（含 /）
grep -oE '`[a-zA-Z_][a-zA-Z_0-9./-]*/[a-zA-Z_0-9./-]+`' docs/SUBSYSTEMS.md \
  | tr -d '`' | sort -u | while read p; do
    # 只检查看起来像真实文件/目录的（含 / 且不是命令片段）
    case "$p" in
      bin/*|/run/*|/gps/*|/SMG4/*|--*) continue;;
      *)
        if [ -e "$p" ]; then echo "OK   $p"; else echo "MISS $p"; fi
        ;;
    esac
done | grep MISS || echo "all paths exist"
```
Expected: `all paths exist`。任何 MISS：要么是 Task 1 漏掉的路径要订正，要么是路径是示例占位符（如 `data/qmd/imqmd_pb208_190mevu.root`、`build/sim/*.root`）——后者在文档里应该明显是示例（用 `<...>` 或 `build/` 路径），如果查出 MISS 且不属于示例，必须修。

- [ ] **Step 3: 检查三个入口命令的 --help 能跑（不要求输出正确，只要 ParseArgs 不段错）**

Run:
```bash
# 前提：已经跑过一次 cmake + make
ls bin/sim_deuteron bin/reconstruct_target_momentum bin/run_qmd_geo_filter 2>/dev/null \
  && echo "binaries exist" || echo "not built yet — skip this check"

# 如果 binaries 存在：
bin/reconstruct_target_momentum --help 2>&1 | head -20
bin/run_qmd_geo_filter --help 2>&1 | head -20
# sim_deuteron 没有 --help，跳过
```
Expected: 两个 --help 输出含 `--input`, `--output`, `--geometry-macro`（reco）/ `--qmd`, `--field`, `--angle`（filter）。若 binary 不存在直接跳过。
...
**Files:**
- 删除：`rk_reconstruction_status_20260416_zh.pdf`（根目录的散落 PDF，应该在 `docs/reports/reconstruction/`）
- 可选：`git log --oneline -10` 检查提交历史是否干净

- [ ] **Step 1: 检查根目录散落的 PDF**

Run:
```bash
ls -la rk_reconstruction_status_20260416_zh.pdf 2>/dev/null
```

若存在且在 `docs/reports/reconstruction/` 下有同名副本：

```bash
diff rk_reconstruction_status_20260416_zh.pdf \
     docs/reports/reconstruction/rk_reconstruction_status_20260416_zh.pdf \
  && rm rk_reconstruction_status_20260416_zh.pdf \
  && echo "removed duplicate root-level PDF" \
  || echo "files differ — do not remove"
```

- [ ] **Step 2: 检查 git 历史**

Run: `git log --oneline -10`
Expected: 看到 Task 2–8 的 7 个 commit（SUBSYSTEMS.md skeleton、§1、§2、§3、§4、§A、README 重写）。

- [ ] **Step 3: 最终 commit（若有清理改动）**

```bash
# 若 Task 10 Step 1 有删除：
git add -u
git commit -m "remove duplicate root-level PDF"
```
...
- [x] 每个任务都有明确的 Files 段
- [x] 代码/内容步骤给出完整 Markdown 而非占位
- [x] 验证步骤有明确的 Expected
- [x] 每个任务都有独立 commit
- [x] 总任务数 10（原子化粒度）
- [x] 覆盖 spec 的所有 §（§0 全局 / §1-§4 子系统 / §A 附录 / README 首屏）
- [x] 引用类型一致（`SolverStatus`, `RkFitMode` 等在 §2 和 README 中拼写一致）
- [x] 所有引用的命令行标志（`--backend`, `--geometry-macro` 等）与 `apps/run_reconstruction/main.cc::ParseArgs` 和 `libs/qmd_geo_filter/src/RunQMDGeoFilter.cc` 的 getopt 表一致

## 超出本计划

- 英文版 README（后续）
- 每个 skill 的 SKILL.md 更新（另一条线）
- `CLAUDE.md` 内容（已从 git 移除）
