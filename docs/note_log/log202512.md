...
why anaroot on the web  is different from anaroot in ribdana04
```
...
qmd 数据是在靶点的局部坐标系中定义的. 需要把动量旋转到实验室坐标系中.
...
但是最后画图 需要画的是靶点的局部坐标系下的动量分布. 
    
qmd数据是不是有问题:

![alt text](assets/log202512/image-12.png)

![alt text](assets/log202512/image-14.png)

![alt text](assets/log202512/image-13.png)