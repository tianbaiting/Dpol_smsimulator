...
test the best step size for trajectory calculation in libs/analysis/src/ParticleTrajectory.cc, using real magnetic field. .about sim_deutron_core and the root file is generated by scripts/event_generator/dpol_eventgen/GenInputRoot_np_atime.cc. Use simsimulator to calculate the trajectory and output to a root file,  and caculate the trajectory by ParticleTrajectory.cc using different step. the diference compared with geant4(sim_deutron) is the  . You may need to modify this script slightly to plot trajectories with different step sizes and energy deposition points.  write a test script to automate this process and compare the results for different step sizes.

the B is 1.15T, 3deg. the proton momentum is (100MeV/c, 0, 627MeV/c) and (-100MeV/c, 0, 627MeV/c). (50MeV/c, 0, 627MeV/c) and (-50MeV/c, 0, 627MeV/c). 

the geant4 mac is /home/tian/workspace/dpol/smsimulator5.5/configs/simulation/DbeamTest/detailMag1to1.2T/run_protons_B115T_3.0deg.mac

to see the difference between sim_deutron and ParticleTrajectory.cc, you can output the trajectory data from sim_deutron to a root file,  and then read this root file in ParticleTrajectory.cc to calculate the trajectory with different step sizes. The difference menans the distance between the pdc deposit position and the trajectory calculated by ParticleTrajectory.cc. Finally, you can compare the trajectories from both methods and analyze the differences.

then visualize it with eventdisplay using scripts/visualization/run_display.C.
